...
def register(app: FastAPI) -> None:
    ...
    # ── PMCC roll_short combo HITL (Phase A, 2026-07-22) ─────────────────
    # A PMCC roll_short is a 2-leg atomic combo (buy-to-close + sell-to-open a
    # strictly-later short) placed via place_combo -> place_multi_leg. Dedicated
    # PMCC registry + routes (the IC path is a precedent to READ, not edit).
    # roll_leap is NOT here — it is advisory-manual and never combo-tagged.

    @app.get("/approvals/pmcc-combos/{combo_id}", response_class=HTMLResponse)
    async def pmcc_combo_approval_detail(request: Request, combo_id: str):
        """Detail page for one pending PMCC roll_short combo. 404 when no longer
        pending (already resolved or never registered)."""
        from trading_corp.web.combo_approval_view import build_combo_card_payload
        registry = deps.pmcc_pending_combo_registry
        if registry is None:
            raise HTTPException(
                status_code=404, detail="pmcc combo registry unavailable",
            )
        entry = registry.get(combo_id)
        if entry is None:
            raise HTTPException(
    ...
    async def pmcc_combo_approval_decide(request: Request, combo_id: str):
        """POST decision endpoint for a PMCC roll_short combo.

        On approve: pops the registry entry, writes `board_combo_approved`, then
        reuses the generic, duck-typed `dispatch_approved_ic_combo` with
        `strategy=deps.pmcc_agent` (atomic place_combo + on_combo_filled). This
        does NOT touch the IC dispatch path. On reject: pops + audits only.
        """
        from trading_corp.agents.strategies._ic_orchestration import (
            dispatch_approved_ic_combo,
        )
        ...
        if decision_str == "approve":
            try:
                fills = await dispatch_approved_ic_combo(
                    entry.orders,
                    strategy=deps.pmcc_agent,
                    data_exec=deps.data_exec,
                    division=entry.division,
                )
            except Exception as e:
                log.exception(
                    "pmcc_combo_approval_decide: dispatch raised for combo %s "
                    "— APPROVED in registry but place_combo failed; see audit",
                    combo_id,
                )
                return HTMLResponse(
                    f'<div class="text-loss text-sm font-mono">'
            ...
            return HTMLResponse(
                f'<div class="text-gain text-sm font-mono">'
                f'Decision recorded: APPROVE · PMCC combo {combo_id[:8]} · '
                f'{leg_count} leg(s) filled.</div>',
            )
        return HTMLResponse(
            f'<div class="text-mono text-sm font-mono">'
            f'Decision recorded: REJECT · PMCC combo {combo_id[:8]}.</div>',
        )
