...
Confluence-gate Phase B adds:
  - `prior_day_session_vwap` — yesterday's full-day VWAP, used by
    Factor 2 of the 5-factor confluence gate.
  - `cvd_from_bars_tick_rule` — close-direction CVD slope for Factor 4.
  - `build_gate_inputs` — orchestrator that reads 3m / 5m / 15m caches
    and produces a `GateInputs` for `evaluate_confluence_gate`.
  - `log_gate_cache_warmup_status` — boot-time warm-up ETA logger
    (Board mod #4). Surfaced so the operator can see whether the
    gate will be fully informed at first fire after restart.
"""
