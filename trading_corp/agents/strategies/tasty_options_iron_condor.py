"""Tasty Options Iron Condor — 45 DTE neutral premium-selling strategy.

Watchlist: SPY, QQQ, IWM (default — operator configures via YAML; the
watchlist is permissive: off-watchlist symbols pasted into the IC grader
proceed through gates 2-8 rather than failing at gate 1).
...
first match wins, per open IC except where noted:
  ...
  4. Ex-dividend within 3 trading days AND short call delta > 0.25 →
     ...
     ETF dividends in the watchlist (SPY/QQQ/IWM) are <0.5% of
     price per ex-date, within normal daily noise. Re-evaluate if the
     watchlist expands to higher-yield names.
...
`(agent='tasty_options_iron_condor', key='state')` carries the open-IC
...
class TastyOptionsIronCondorAgent:
    """45 DTE iron-condor strategy for the Tasty Options division.
    ...
    SLUG = STRATEGY_SLUG
    # IC grader gate-1 knob: when False, off-watchlist symbols pasted into
    # the grader proceed through gates 2-8 (live-data checks) and get a
    # `watchlist_membership=off` tag rather than failing at gate 1. This
    # is the "Tasty Options watchlist is permissive" semantic vs RH Joint's
    # hard-gate universe.
    ...
    def division(self) -> str:
        self._reload()
        return str(self._cfg.get("division", "tasty_options"))

    @property
    def watchlist(self) -> list[str]:
        self._reload()
        val = self._cfg.get("watchlist")
        # Distinguish None/missing (fall back to defaults) from [] (intentionally empty).
        if val is None:
            return list(_DEFAULTS["watchlist"])
        return list(val)

    @property
    def universe(self) -> list[str]:
        """Alias for `watchlist` — kept so IC grader's _StrategyLike protocol
        (which expects `universe`) reads the watchlist as the membership set."""
    ...
    async def scan(
        ...
        if not wl:
            log.info(
                "%s.scan: watchlist is empty — skipping daily scan "
                "(paste-only entry via /telemetry/iron_condor)", STRATEGY_SLUG,
            )
            # Audit row: scan_skipped_empty_watchlist
            try:
                ts = self._clock().isoformat(timespec="seconds")
                payload_json = json.dumps({"reason": "watchlist_empty"})
                with db.connect(self._db_url) as conn:
                    conn.execute(
                        "INSERT INTO audit_event(ts, actor, kind, payload_json) "
                        "VALUES(?,?,?,?)",
                        (ts, STRATEGY_SLUG, "scan_skipped_empty_watchlist", payload_json),
                    )
            except Exception as e:
                log.debug("scan_skipped_empty_watchlist audit emit failed: %s", e)
            return []
        ...
        if self._macro.is_within_halt_window(
            ...
            self._tally_scan_filter(state, self.watchlist, "macro_halt")
            self.persist_state(state)
            log.info("IronCondor scan: macro halt window — skipping all symbols")
            return []
        vix = get_vix()
        if vix is not None and vix > 30:
            self._tally_scan_filter(state, self.watchlist, "vix_above_30")
            self.persist_state(state)
    ...
    async def _evaluate_ic(
        ...
        if self._exdiv.is_within_window(
            ...
            # dividends in our watchlist are <0.5% of price per ex-date
            # — within normal daily noise. Re-evaluate if the watchlist
