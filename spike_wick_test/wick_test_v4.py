"""Wick-Test v4 (GROSS R) - v3 with the trend filter swapped to a 20-bar PRICE-SLOPE
directional filter (long iff slope20>0, short iff slope20<0). Single-variable change from v3.
See PRE_REGISTRATION_v4.md. Read-only; k=1 causal; 3m; 4 coins.
...
ATR_N = 14
SLOPE_N = 20                  # 20-bar price-slope trend filter (the v4 change)
KS = [1.0, 1.5, 2.0]
TARGET = 2.0
DR_WINDOW = 3
LIT_BUFFER = 0.0005
...
def slope20(closes):
    """OLS slope of close over the last SLOPE_N bars (causal). None until warm.
    slope>0 = local uptrend, <0 = downtrend."""
    N = SLOPE_N; xs = list(range(N)); xbar = (N - 1) / 2.0
    sxx = sum((x - xbar) ** 2 for x in xs)
    out = [None] * len(closes)
    for i in range(N - 1, len(closes)):
        ys = closes[i - N + 1:i + 1]; ybar = sum(ys) / N
        num = sum((xs[k] - xbar) * (ys[k] - ybar) for k in range(N))
        out[i] = num / sxx
    return out
...
def gen_setups(m3, side, atr, slp, reg3):
    ...
    for b3 in range(WARMUP, n):
        ...
        for f in range(b3 + 1, min(b3 + DR_WINDOW, n - 1) + 1):
            if (side == "long" and l[f] <= L) or (side == "short" and h[f] >= L):
                fill = f; break
        sl = slp[b3]
        with_trend = (sl is not None) and ((sl > 0) if side == "long" else (sl < 0))
        setups.append({"b3": b3, "L": L, "atr": atr[b3], "b3low": l[b3], "b3high": h[b3],
                       "fill": fill, "slope_ok": with_trend, "regime": reg3[b3]})
...
def build(setups, side, stopcfg, filt, m3, target=TARGET):
    ...
    for s in setups:
        ...
        if filt == "slope" and not s["slope_ok"]: continue
        entry = s["L"]
...
def null_p95(m3, side, stopcfg, target, mode, n_target, atr, rng, slp=None, slope_dir=None):
    """Same-geometry direction-matched null. If slope_dir given (+1 long / -1 short), draw
    ONLY from bars whose slope20 matches (slope-matched null for the slope-filter cells)."""
    ...
    valid = [i for i in range(n - 2) if atr[i] is not None and atr[i] > 0
             and (slope_dir is None or (slp[i] is not None and (slp[i] > 0) == (slope_dir > 0)))]
...
def main():
    ...
    print("=" * 108)
    print("WICK-TEST v4 - GROSS R - v3 + 20-bar price-slope trend filter. See PRE_REGISTRATION_v4.md")
    print("=" * 108)
    data = {}
    for coin in COINS:
        m3 = load(coin, "bars_3m"); m15 = load(coin, "bars_15m")
        cts, lab = regime_labels_15m(m15); reg3 = regime_3m(m3, cts, lab)
        atr = atr14(m3); slp = slope20(m3["c"])
        days = (m3["ts"][-1] - m3["ts"][0]) / 86_400_000
        drift = (m3["c"][-1] / m3["c"][0] - 1) * 100
        mrp = median([1.5 * atr[i] / m3["c"][i] * 100 for i in range(m3["n"]) if atr[i]])
        data[coin] = dict(m3=m3, reg3=reg3, atr=atr, slp=slp, days=days, drift=drift, mrp=mrp)
        print(f"[{coin}] 3m n={m3['n']} ({days:.1f}d) drift={drift:+.1f}% medR-unit(1.5ATR)={mrp:.3f}%ofpx")

    setups = {}; trig = {}
    for coin in COINS:
        d = data[coin]
        for side in SIDES:
            setups[(coin, side)], trig[(coin, side)] = gen_setups(d["m3"], side, d["atr"], d["slp"], d["reg3"])

    print("\n" + "=" * 108)
    print("DR FILL SELECTIVITY + slope-with-trend pass rate")
    print("=" * 108)
    for coin in COINS:
        ...
        for side in SIDES:
            ...
            slp_ok = sum(1 for s in su if s["fill"] is not None and s["slope_ok"])
            fr = filled / t * 100 if t else float("nan")
            print(f"  {coin} {side:5s}: triggered={t:4d} filled={filled:4d} ({fr:4.1f}%, {filled/wk:4.1f}/wk) "
                  f"slope-with-trend={slp_ok:4d} ({slp_ok/filled*100:4.1f}% of fills)")

    print("\n" + "=" * 108)
    print("MAIN GRID - GROSS R  (2R; BODY=record, HARD=contrast; filter slope = long slope>0 / short slope<0)")
    print("  null for slope cells is SLOPE-MATCHED (random entry from same-slope bars) - isolates pattern vs slope")
    print("=" * 108)
        ...
        for side in SIDES:
            ...
            for sc in STOPCFGS:
                for filt in FILTERS:
                    ...
                    shk = f"{len(so)/b['n']*100:4.1f}% {netval:+6.1f}"
                    thin = "*" if 0 < b["n"] < GATE_MIN_N else " "
                    print(f"  {coin:7s}{side:5s}{scn:7s}{filt:6s}{b['n']:>4d}{thin}| "
                          f"{bcell:^32s} | {hcell:^22s} | {shk:^14s}")

    print("\n" + "=" * 108)
    print("DR-SKIP DIAGNOSTIC - would-have-been R at bar-3 close (BODY, 2R), filter=none")
    print("=" * 108)
    for k in KS:
        pooled = []
        for coin in COINS:
            ...
            for side in SIDES:
                for s in setups[(coin, side)]:
                    ...
                    line = entry - rp if side == "long" else entry + rp
                    tp = entry + TARGET * rp if side == "long" else entry - TARGET * rp
                    pooled.append(sim_body(side, entry, line, tp, TARGET, rp, s["b3"] + 1, d["m3"])[0])
        print(f"  k={k}: skips={len(pooled)} would-have-been avgR(body)={mean(pooled):+.4f}")
    ...
    print("\n" + "=" * 108)
    print("BEAR-BETA CONTEXT - window drift + passive-short R-equiv")
    print("=" * 108)
    for coin in COINS:
        d = data[coin]
        pshort = (-d["drift"] / 100) / (d["mrp"] / 100)
        print(f"  {coin}: drift={d['drift']:+.1f}%  passive-short whole-window ~{pshort:+.1f}R (1.5ATR units)")
