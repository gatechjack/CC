...
| `bitunix_futures` | BitUnix Futures (USDT + USDC margined) | None today (Phase 3 will wire division-entry filters + signal fan-out) | Read-only Phase 1 SHIPPED 2026-05-03 17:54 UTC. Live `place_order` raises `NotImplementedError` until Phase 4 (gated on stop-loss strategy + conviction → leverage map). See `~/.claude/.../memory/trading_corp_bitunix_vision.md`. **Equity double-count bug (BACKLOG P2, 2026-05-09):** `bitunix.py:213-225` adds `transfer` on top of `available`; UI shows 2× actual cash. Display-only today; promotes to P0 before Phase 4 since sizing math reads the value. |
| `polymarket_arbitrage` | Polymarket prediction markets (single dedicated EOA wallet on Polygon mainnet, signer == funder) | **polymarket_arbitrage** ([strategies/polymarket_arbitrage.py](trading_corp/agents/strategies/polymarket_arbitrage.py)) — scan-driven LLM-divergence detector. Pulls open Polymarket markets via gamma-api, deterministic-filters by volume/spread/ttr/implied-prob, K=10 survivors per cycle get a calibrated YES probability via direct Anthropic call (NOT through Research firm; shared analyst-persona system prompt at `_polymarket_prompts.py` is prompt-cached). Emits ProposedOrder when `\|LLM prob - implied prob\| × 100 ≥ 10%`. | Read-only Phase 1+2a SHIPPED 2026-05-09/10. Wallet live ($500 USDC). Strategy `enabled: false` until gamma-api query is tuned + Phase 2.5 backtest verdict clears. See `~/.claude/.../memory/trading_corp_polymarket.md`. |
