...
$cmd = @'
cd /home/azureuser/trading_corp; open=$(sqlite3 data/trading_corp.db "SELECT COUNT(*) FROM paper_trade_record WHERE division='bitunix_sfp' AND result IS NULL"); age=$(( ($(date +%s) - $(stat -c %Y ~/.tokens/robinhood.pickle 2>/dev/null || echo 0)) / 3600 )); echo "SFP open live rows=$open ; RH pickle age h=$age"; if [ "$open" != "0" ]; then echo "ABORT: SFP NOT flat ($open open rows) -- not restarting"; exit 2; fi; if [ "$age" -gt 20 ]; then echo "ABORT: RH pickle STALE ($age h) -- run rh_pickle_refresh.ps1 FIRST (whole-engine 2FA hang)"; exit 3; fi; echo "GATES PASS -> ONE restart"; sudo -n systemctl restart trading-corp; sleep 6; echo "post:"; systemctl show trading-corp -p MainPID,NRestarts,ActiveState,SubState
'@
