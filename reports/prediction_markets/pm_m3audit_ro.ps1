...
$sh = Join-Path $PSScriptRoot "pm_m3audit_ro.sh"
...
function Find-Exe($name) {
  ...
  throw "$name not found (STANDING BOX QUIRK -- 32-bit process? try a 64-bit shell)."
}
$ssh = Find-Exe "ssh"; Write-Host "Using ssh=$ssh"
Write-Host "PM M3-clobber audit (read-only) on $h ..."
Get-Content -Raw $sh | & $ssh -o ConnectTimeout=15 -o ServerAliveInterval=20 $h "tr -d '\r\357\273\277' | bash"
Write-Host "---- m3 audit exit ----"
