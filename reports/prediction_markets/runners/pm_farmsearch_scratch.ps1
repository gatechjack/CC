...
$tar = "C:\Users\AA Incorporado\cc\pm_farmsearch_overlay.tar"
$sh = Join-Path $PSScriptRoot "pm_farmsearch_scratch.sh"
...
# git archive HEAD -> LF tar of my coherent worktree slice (stored blobs = LF, not the autocrlf worktree)
& git -C $wt archive -o $tar HEAD trading_corp tests pyproject.toml
if (-not (Test-Path $tar)) { throw "git archive produced no tar" }
Write-Host ("overlay tar: {0} bytes -> scp to box" -f (Get-Item $tar).Length)
& $scp -o ConnectTimeout=20 $tar ("{0}:/home/azureuser/pm_farmsearch_overlay.tar" -f $h)
Write-Host "Streaming farm-search box-scratch (READ-ONLY; scratch tree; live untouched) to $h ..."
