...
$sh = "C:\Users\AA Incorporado\cc\_p2_probe.sh"
...
Write-Host "[remote] running pre-CP1 runner (read-only to prod)..."
Get-Content -Raw $sh | ssh -o StrictHostKeyChecking=accept-new $h "tr -d '\r\357\273\277' | bash"
Write-Host ("[done] pm_p2 pre-CP1 runner finished (ssh exit " + $LASTEXITCODE + ")")
