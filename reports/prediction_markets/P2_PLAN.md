...
## 5. New tables (additive migrations, NUMBERED ON LANDING — §2) + where each analytic computes

> **AMENDMENT 2026-08-25 (CP3b gate — RULING §2: NUMBER ON LANDING, NEVER RESERVE).** CP3a shipped 004–006
> (live schema is now **6**). No document reserves a number for a table not yet written — reserving in advance
> is exactly what produced the 006-vs-005 error. `pm_paper_category_stats`, `pm_search_run`, `pm_analysis_cache`
> each take **the next integer when their migration LANDS** (007, 008, … in landing order; `pm_paper_category_stats`
> is 007 iff it lands first). Every "before/at 007" in this doc is rebound to the **table name**, not a number.

Follows `db.py`'s exact numbered/idempotent pattern (`MIGRATIONS += [(4,…),(5,…),(6,…),(N,…)]`, `IF NOT EXISTS`, one txn each, `schema_version` bump). **No existing table is rebuilt** (no PK change needed). Test literals in `test_db.py` bump to the landed head. **Auth tables are NOT created (Ruling C).**
...
```
> **AMENDMENT 2026-08-25 (CP3b contamination gate — Finding A).** The lifecycle sentence below was written
> from expectation and does NOT match what CP3a SHIPPED; the shipped `paper.py@b593b39` governs (see
> `CP3A_DEPLOY_COMPLETE.md`). Corrections:
> - **ENTRY = `/positions` OBSERVATION, not an `/activity` BUY.** `entry_basis='positions_observation'`
>   (db.py:377, written at paper.py:255); the poller `poll_pinned` reads `fetch_positions` (paper.py:145) —
>   there is NO `/activity` read anywhere in the poller.
> - **EXIT = TWO-PHASE, not a direct vanish→stale.** A pinned position that vanishes from `/positions` before
>   resolving → `status='pending_adjudication'` (paper.py:229); `adjudicate()` (off the `/closed-positions`
>   refresh) then decides `closed | stale`.
> - **STATUS ENUM = FIVE values:** `open | pending_adjudication | closed | stale | void` (db.py:385). A rollup
>   built from the pre-CP3a text below would have FOUR statuses where the code has five — silently dropping
>   every row while it sits in `pending_adjudication`. `paper_rollup` MUST derive over the shipped five-status
>   model, not this sentence.
>
> *(Pre-CP3a text, kept for history:)* Lifecycle in a **new** `prediction_markets/paper.py` (additive; imports §3A helpers from `ingest.py`, does NOT edit it): **OPEN** = paper-entry job (§7.2) inserts on a pinned whale's new `/activity` BUY in the pinned category; **MARK/CLOSE/STALE** = weekly refresh (resolution is not time-critical; a whale with an open paper position that no longer appears in `/positions` and has not resolved → `status='stale'`); **paper_rollup** mirrors `stats.rollup()` over `status='closed'` scoreable rows → `pm_paper_category_stats`. Entry-basis recency becomes real: `compute_scores(..., recency_basis='entry_ts')` over `(won, entry_ts)`.

### 5.3 Migration 006 — roster + watchlist (search-run DEFERRED — see the amendment + §2 numbering) (Ruling B)
...
>    reference only.
> 2. **Category attribution — RULED 2026-08-24 by Jack (advisor ruling C2.4 REVERSED).** The pinned-paper set
>    (all seeded `status='pinned'`) is **EVERY `(wallet, category)` pair in `pm_category_stats` for the migrated
>    legacy whale set** — with **NO minimum-resolved floor** (an n=3 pair is a pinned pair; curation is the
...
-- 006: per-category CANDIDATE list (CP3b-0 rename 'watchlist'->'candidate'). status='candidate' = Analyze-able, NOT paper. status='pinned' =
...
CREATE TABLE IF NOT EXISTS pm_watchlist (
    ...
    status TEXT NOT NULL DEFAULT 'candidate',     -- 'candidate'|'pinned' (CP3b-0 rename; live DB keeps historical 'watchlist' default, inert)
...
### 5.4 `pm_analysis_cache` — analyze cache (own migration, NUMBERED ON LANDING per §2 — NOT reserved as 007)
> **AMENDED 2026-08-25 (CP3b-2, AS-BUILT — this DDL is superseded; `db.py` migration 007 is authoritative).**
> The shipped table lands as **007 by number-on-landing** (Analyze adds its table before Search / paper-scoreboard,
> so 007 is genuinely the next integer — not reserved). The shipped shape differs from the proposal below in two
> ruled ways: **(a)** the PK is **`(wallet, category, skill_version)`** — the `input_hash(max(resolved_ts)|model|
> prompt|params)` + `activity_max_ts` design is dropped. `skill_version` is a single bump-able string that folds
> model + prompt + report-shape into ONE invalidation axis and **replaces the legacy 24h TTL** (Jack ruling); there
> is no `activity_max_ts` because there is no `/activity` read (see §7.4). **(b)** A companion **`pm_analysis_cost`**
> (day_utc PK, usd, n_calls) is the ONE visible per-UTC-day spend counter — the daily strip is a `SELECT` on it, not
> a `SUM` over the cache. Only a SUCCESSFUL verdict is cached; reasoned-nulls are not stored. See `db.py` for the
> exact columns.
```sql
-- SUPERSEDED PROPOSAL (kept for provenance; the AS-BUILT is db.py migration 007):
...
- **CANDIDATES** (`pm_watchlist.status='candidate'` ⋈ stats): the scoreboard product columns + per-row **[Analyze] [Pin]**. `[Analyze]`→ POST `/analyze/{wallet}` swaps a sibling `<tr>` (the ANALYZE precedent). `[Pin]`→ POST `/farm/{category}/pin/{wallet}` flips `pm_watchlist` to `pinned` + seeds the paper record.
...
### 7.4 ANALYZE rewire
> **AMENDED 2026-08-25 (CP3b-2, AS-BUILT — supersedes the paragraph below where they conflict). `trading_corp/prediction_markets/analyze.py` is authoritative.**
> - **FORK, not "reuse the live route's shape".** The legacy analyzer is ENGINE code serving the LIVE PCT division; pm_web is standalone (zero engine imports) and the platform is zero-engine-edits, so the narration guardrails + gate structure are **copied** into the PM package. No import of `agents/*` or `data/polymarket_whale_audit.py` (Jack ruling Q6).
> - **`pm_closed_position` ONLY — NO `/activity`, NO network fallback, NO hybrid** (Q3). pm_closed_position has no fills, so the legacy fill-level analytics (clustering, sell-footprint, edge-profile, REDEEM-grounded realized PnL) are DROPPED and `CategoryConcentrationReport` is NOT ported (category is the FILTER here, not a derived metric). The verdict is the leaner P&L / win-rate / **cost-ROI** record, recomputed FRESH through the ONE §3A scoreable predicate + the SAME formulas as `stats.rollup` (so it matches the scoreboard; a stale rollup is surfaced by a reconciliation note, never silently). Category filter = a per-pair `WHERE wallet=? AND category=?`; n-counts are per-pair.
> - **Cache key = `(wallet, category, skill_version)`** (§5.4 as-built), `skill_version` REPLACING the legacy 24h TTL; `?force=1` evicts + re-analyzes. **Only successful verdicts are cached** — reasoned-nulls recompute, so the moment the ANTHROPIC key is wired the next analyze narrates fresh instead of serving a stale `llm_unavailable`. A cache HIT spends nothing.
> - **★ Cost cap = $20/day (`PM_ANALYZE_DAILY_CAP_USD`), Jack ruling 2026-08-25 — supersedes BOTH prior numbers, which disagreed with each other:** this doc / OQ-6 said **$2/day**, but the legacy CODE default (`DEFAULT_DAILY_COST_CAP_USD`) is **$1.00**. Neither was authoritative; the doc was wrong about the code. Spend is recorded in `pm_analysis_cost` (ONE per-UTC-day counter in the PM DB — PM must NEVER write `agent_state`). Over the cap → `daily_cap_hit` reasoned-null (deterministic report still renders).
> - **Five reasoned-nulls, never silent:** the FOUR legacy LLM gates (`disabled_by_flag` · `daily_cap_hit` · `llm_unavailable` · `llm_error`) preserved verbatim, PLUS a DATA-level `no_resolved_positions` (the "refuses honestly at zero rows" case), which by gate-order takes precedence over `llm_unavailable` — an empty slice refuses on data, not on the missing key.
> - **The key is NOT wired (e3, Jack's hands).** `is_llm_available()` reads `ANTHROPIC_API_KEY`, which does not resolve in pm_web today (proven read-only 2026-08-25), so every analyze returns `llm_unavailable` and renders the deterministic report without a verdict. **An importable langchain/azure library is capability, not a working token** — the e3 gate (prove-KeyVault-from-2nd-unit) stays UNPROVEN.
> - **Button** on BOTH the Pinned and Candidate lists, SAME path `POST /farm/analyze/{wallet}/{category}` (identity = the pair, not the list).

Reuses the live analyze route's shape (cache → miss → build audit → narrate → cache → render partial swapped into a sibling row), with three changes: **(1)** read the already-ingested `pm_closed_position` rows (untruncated) instead of `/activity` (network fallback only if the wallet isn't ingested yet); **(2)** cache in `pm_analysis_cache` keyed on `input_hash(max(resolved_ts)|model|prompt|params)` — invalidates exactly when new resolutions land; `?force=1` evicts; **(3)** visible cost — per-call fields already exist; add a daily strip `SUM(llm_cost_usd) WHERE computed_ts≥midnight` and a **cost-guard**: over `PM_ANALYZE_DAILY_USD` (**default $2**, Ruling OQ-6 — $5/day ≈ $150/mo would exceed the entire current ~$60/mo platform spend after Jack's Anthropic cost workstream; $2/day ≈ $60/mo ceiling), skip the LLM narration and render the free computed audit only (reasoned-null, `null_reason` set — never silent). Runs on candidate AND pinned. Runs in `pm_web`, off the engine; no `audit_event` write (that's the engine's table; `pm_analysis_cache` is the PM ledger).
...
- **Migrations** apply idempotently on a fresh DB and on a P1 DB; re-run is a no-op; `schema_version` → the landed head (**6** after CP3a; +1 per CP3b table on landing, §2).
...
- **Paper (SHIPPED model — CP3b gate Finding A):** open from a **`/positions` observation** (`entry_basis='positions_observation'`), scale-in/reduction touch, vanish→**`pending_adjudication`**, then `adjudicate()` off `/closed-positions` → **`closed`|`stale`**; `paper_rollup` counts `n_closed`/`n_open`/**`n_stale`** over the **five-status** enum correctly; entry-basis recency uses `entry_ts`. (NOT the pre-CP3a `/activity`-BUY / direct-stale model.)
...
- [ ] Product tabs (scoreboard; farm league w/ candidates + pinned-paper per category; whale detail; search) + diagnostics all return 200 and honest-empty on an empty DB.
...
- **OQ-8 (recency default):** pinned-paper lists default to **entry_ts**; external candidate/scoreboard to **resolved_ts**; each labeled.
