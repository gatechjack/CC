...
- **`pm_closed_position`** — **PK (wallet, condition_id)**; slug, event_slug, title, **category, category_source**, outcome, outcome_index, avg_price, total_bought, realized_pnl *(CAN BE NEGATIVE)*, cur_price, **won = cur_price≥0.9** (stored at ingest), **`pnl_suspect` (0/1 — the §3A group-aware quarantine flag, computed + stored at ingest) + `suspect_reason` (NULL | 'row_invariant' | 'event_group')**, shares_derived (=total_bought/avg_price, NULL-safe), end_date, resolved_ts (unix), ingested_ts, updated_ts. Indices: (wallet), (category), (wallet, resolved_ts DESC), (wallet, won, category), **(wallet, category, pnl_suspect)** [serves the §3A scoreable predicate].
...
> **★★ ROI DENOMINATOR IS WRONG — BLOCKING, needs Jack's ruling (2026-08-22, PROVEN read-only, `ROI_DENOMINATOR_FINDING_2026-08-22.md`, §13A(g)).** `/closed-positions total_bought` is the **NOTIONAL (share count = payout at $1), NOT the USDC cost**. Proven to the dollar: `total_bought * avg_price == /activity BUY` (control-win cost-vs-buy -0.3%/-4.8%; the ~-45% notional-vs-cost gap is UNIVERSAL across all slices). So `roi = realized / total_bought` below is **return-on-notional** — it understates true return-on-cost by ~avg_price and DISTORTS ranking (longshot bettors penalized ~1/avg_price). **PROPOSED fix (do NOT implement until ruled):** `roi = net_realized / SUM(total_bought * avg_price)`; `avg_bet` likewise; store `cost_basis`. `avg_price` is already on every row -> one-line rollup change. Ranking stays GATED until ruled; INGESTION is unaffected (raw fields stored faithfully). "scale-ins pre-collapsed" (below) is part of this same notional/cost accounting, not a separate cleanliness win.
