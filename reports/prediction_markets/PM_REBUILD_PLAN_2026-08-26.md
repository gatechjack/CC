...
> **★★★ STAGE 0 CLOSED — COMPLETE + DEPLOYED LIVE 2026-08-27.** All three rungs done: **rung 1** (migration 008, 02:15Z) · **rung 2** (5-file deploy + pm_web restart 40483→132990, 12:33–12:49Z) · **rung 3** (22-row `active=0` write, 13:10:36Z). **LIVE NOW: PM DB schema 8; 92 active / 22 inactive; 15 categories / 92 pairs; `/farm` 182,835 bytes; engine PID 89366 / pm_web PID 132990.** `origin/prod-live` advanced `95e78c4 → c77f618` (fast-forward). Branch `prediction-markets-stage0-2026-08-26` @ `df1300b`. Execution records: `RUNG2_DEPLOY_COMPLETE_2026-08-27.md` + `RUNG3_STAGE0_CLOSEOUT_2026-08-27.md`; end-of-Stage-0 handoff: `TRANSITION_STAGE0_COMPLETE_2026-08-27.md`. **Stage 1 NOT authorized, NOT prepared.** The dated header and every "PLAN ONLY / BUILT / NOT deployed / NOT yet applied / live schema 7" statement below describes the **2026-08-26 PLANNING state** and is **SUPERSEDED for Stage 0** (kept as build history, not current fact).

**Date:** 2026-08-26 (plan authored) · **Branch context (plan-time):** `prediction-markets-cp3b2-2026-08-25` @ `f4fb61d` · live schema **7 → NOW 8 (Stage 0 complete)** · `origin/prod-live 95e78c4 → NOW c77f618`.
...
Migrations are **numbered on landing** — ~~live is at schema 7~~ **live is at schema 8 (migration 008 APPLIED + DEPLOYED, Stage 0 complete 2026-08-27)**. Stage 0 built + landed migration 008 (`pm_watchlist.active`), so the downstream numbers are FIXED: **Stage 1 = 009, Stage 4 = 010, Stage 5 = 011.** If landing order still changes, the numbers follow the landing order, not this plan. Every stage carries the CP3a gate rhythm: **build → box-scratch (pytest actuals + render/verify against a WAL-safe copy of live) → report with SHA → HALT → Jack authorizes deploy separately** (code deploy + any pm_web restart on the root `az vm run-command` channel; poller/adjudicator/cron runs + any live-DB write are separate Jack-authorized acts).
...
**Mechanism — BUILT + box-scratch GREEN 2026-08-26; DEPLOYED LIVE 2026-08-27 (Stage 0 complete)** (branch `prediction-markets-stage0-2026-08-26`; the "live untouched / NOT deployed" note below was the 2026-08-26 build state). **ONE removal path for all 22:**
...
- **The 22, with the THREE DISTINCT reasons recorded per category** (§F-2). **★ DEPLOYED VALUES (rung 3, 2026-08-27) = the SHORT strings** `not_probed` (cbb×3) · `dormant_calendar` (fifwc×8) · `structural` (unknown×11), per Jack's rung-3 authorization and the rung-3 line below. The longer variants once written here (`pending_analysis_ncaab_not_probed` / `dormant_calendar_returns_next_wc` / `structural_slug_failure`) are **SUPERSEDED** — the live rows carry the short strings.
...
- **Rung 1 — apply migration 008 to live via `init_db()` from an EPHEMERAL SCRATCH EXTRACT (the Gate-1 mechanism), NOT by deploying `db.py`.** This is exactly how CP3b-2 Gate 1 applied migration 007 (`CP3B_DEPLOY_COMPLETE.md:28`): byte-verify the extract `== <branch SHA>`, run its `init_db()` against the LIVE PM DB; the runtime `db.py` stays untouched until rung 2. 008 lands once, explicitly, under control.
  - **★ init_db auto-migrate — every trigger, and why the scratch extract closes the boundary:** PM `db.init_db()` auto-applies pending migrations and is called ONLY by `pm_cli` subcommands — `backfill`/`refresh` (`pm_cli.py:55`), `rollup` (:67), `paper-poll` (:129), `paper-adjudicate` (:140), `migrate-roster` (:157), `analyze` (:180). **pm_web does NOT call `init_db` on startup** (`web/app.py` builds `FastAPI(...)` with no startup/lifespan hook) → **the rung-2 pm_web restart is NOT a migration trigger.** `main.py:293`'s `init_db` is the ENGINE's (`persistence/db.py` on `trading_corp.db`) — a different function, never touches the PM DB (isolation). The UNATTENDED triggers are now the **FOUR PM cron jobs (installed 2026-08-27): poller `*/30`, refresh `0 5`, adjudicate `40 5`, rollup `50 5`** (was: only the single 03:20 refresh — SCHEDULE CHANGED, refresh moved 03:20→05:00 + poller/adjudicate/rollup added; see the cadence section). All other `pm_cli` calls are manual. **Because rung 1 did NOT deploy `db.py`, the runtime kept the OLD `db.py` → its `init_db` was a harmless no-op → nothing auto-migrated unattended.** Any future unattended-write concern must now account for all four cron jobs, not one.
...
- **PRE-1 Timing — clear of the PM cron. ★★ SCHEDULE CHANGED 2026-08-27: the refresh MOVED 03:20 → 05:00 UTC, and the poller now runs `*/30` (writes every :00/:30) + adjudicate 05:40 + rollup 05:50 — see the cadence section. So a future deploy window must be clear of 05:00 (refresh) AND not straddle a `*/30` poll, not "clear of 03:20".** ★ LOAD-BEARING, not tidiness — here is WHY:** the byte-identical
...
**★ POLLER/ADJUDICATOR CADENCE — INSTALLED LIVE 2026-08-27 (azureuser crontab; see `CADENCE_INSTALL_2026-08-27.md`).**
**★★ SCHEDULE CHANGED: the daily refresh MOVED 03:20 → 05:00 UTC** (Jack's ruling — the ~22-min refresh gave adjudicate/rollup no clean separation at 03:20). **Every window-check from now on uses 05:00, not 03:20** (older dated records that say "03:20" are HISTORY — the cron WAS at 03:20 through 2026-08-27).
- **Poller** `pm_cli paper-poll` — `*/30 * * * *` (:00, :30) → `pm_poll.log`.
- **Daily refresh** `pm_cli refresh --cap 50000` — **`0 5 * * *` (05:00 UTC, MOVED from 03:20)** → `pm_refresh.log`; runs ~22 min, done ~05:22.
- **Adjudicate** `pm_cli paper-adjudicate` — **`40 5 * * *` (05:40)** → `pm_adjudicate.log` (~18 min after the refresh completes; no poll at :40).
- **Rollup** `pm_cli paper-rollup` — **`50 5 * * *` (05:50)** → `pm_rollup.log` (~10 min after adjudicate; no poll at :50).
- **Ordering (load-bearing, held):** **poll → adjudicate → rollup** — the 05:30 `*/30` poll precedes the 05:40 adjudicate precedes the 05:50 rollup; the 05:00 refresh completes before adjudicate. All 4 jobs are azureuser crontab, `>> log 2>&1`.
- **★ CONCURRENCY (from the code, `CADENCE_INSTALL` §): there is NO single-writer guard** (no `flock`, no `fcntl`/`filelock`, no `BEGIN IMMEDIATE`) — **schedule spacing is the only protection.** `db.connect` is WAL + `busy_timeout=5000`; two writers serialize on WAL's single write-lock (2nd waits ≤5s then `SQLITE_BUSY`). All PM writes are short (refresh per-wallet, poll per-whale, adjudicate/rollup one sub-second commit), so the only overlap — the daily 05:00 poll vs 05:00 refresh — is absorbed silently; WAL guarantees no corruption / no silent partial. A proper guard (`flock -n pm_cron.lock` per line) is **NOT built — Jack's ruling pending.**

**★ STAGE-1 RUNG LADDER (Stage 0's shape; NOTHING below is authorized):**
- **Rung 0 — BOX-SCRATCH GREEN (the gate).** Copy the branch PM package + `pyproject.toml` + `tests/conftest.py` + `tests/prediction_markets/` to a box `/tmp` scratch (the pytest config + shared conftest are REQUIRED — omitting `pyproject.toml`'s `asyncio_mode=auto` false-fails the async suite); `venv/bin/python -m pytest tests/prediction_markets/ -p no:pytest_ethereum -q` GREEN; prove migration 009 (PURE DDL) by `db.init_db` on a **`.backup` copy** of live (schema→9, `pm_paper_category_stats` created, **NO config row written**) with the **live DB byte-untouched (still schema 8)** and engine/pm_web PIDs unchanged. Runner `cc\pm_stage1_boxscratch.*`. STOP if any test fails. **(RUN 2026-08-27: first pass GREEN except the 6 head-pin bumps + the 1 grace-in-migration failure — both fixed this pass; re-run pending.)**
- **Rung 1 — migration 009 → live** (like Stage-0 rung 1: `init_db` from a byte-verified scratch extract; runtime `db.py` untouched). **009 is PURE DDL — it writes NO config row.** PRE: DB backup (Gate-1 online-backup: path + sha256 + `integrity_check`), window clear of 03:20 cron, PIDs captured. POST: live schema 8→9; `pm_paper_category_stats` present + EMPTY; pm_watchlist 92/22 unchanged; pm_paper_trade 102; grace config STILL 172800 (unchanged by 009); `/farm` UNCHANGED (old runtime ignores 009); pm_web PID unchanged. STOP on any deviation.
- **Rung 1b — grace 72h live config write (explicit, separately-verifiable; Jack RULED 72h, FIX-2 option ii).** A one-shot authorized DB write, immediately after rung 1: `UPDATE pm_paper_config SET value='259200', updated_ts=<int epoch> WHERE key='grace_window_sec'` (72h; 259200 = 72×3600). **What it writes:** exactly one row's `value` (48h→72h) — no schema change, no other table. **POST-check:** `SELECT value FROM pm_paper_config WHERE key='grace_window_sec'` == `'259200'` AND `paper.get_config(conn,'grace_window_sec')` == `259200.0`; row count of pm_paper_config unchanged (still 3); nothing else in the DB moved. **Backup:** the rung-1 DB backup covers it (taken just before). Reversible: `UPDATE … value='172800'`. STOP if the value is anything but 259200 or any other row changed. (This is the visible authorized step that replaces the migration-embedded re-seed.)
...
- **Rung 3 — run poller + adjudicator + install cadence** (separate Jack authorizations; live DB writes). One-shot `paper-poll` → `paper-adjudicate` → `paper-rollup` in a calm window, then install the crons above. This is where paper numbers begin to accrue.
  - **★ CORRECTED ORDER + WHY (load-bearing — poller is FIRST because it is the ONLY producer of `pending_adjudication`).** `adjudicate()` processes **only** `WHERE status='pending_adjudication'` rows; the **poller is the sole thing that transitions a trade there** (`poll_pinned` marks a vanished open position `pending_adjudication`). **Confirmed on live (2026-08-27):** before the first poller run the live DB had **102 paper trades, all `open`, 0 pending** — so `paper-adjudicate` run first would have been a **confusing live no-op (0 rows)**, not an error. The order is therefore not a preference: **poll → adjudicate → rollup** because poll *creates* the pending rows adjudicate consumes and rollup reads the `closed` rows adjudicate produces. Any cadence, and the one-shot unstick, MUST start with the poller.
  - **★ STEP 1 (adjudicator gamma dry-run) + STEP 2 (one manual poll) EXECUTED 2026-08-27** (`STAGE1_RUNG3_ADJUDICATOR_DRYRUN_2026-08-27.md`, `STAGE1_RUNG3_POLL_2026-08-27.md`). Step-2 poll: 92 pairs (active gate held, 0 deactivated polled/traded), +5 game-line captures (mlb·2/ufc·2/cs2·1), 3 open→pending (2 resolved ufc + 1 mlb whale-exit), 0 errors, 2.94s (→ `*/30` trivially safe). **Step 3 = the live `paper-adjudicate` + `paper-rollup`; step 4 = install the crons. Both remain unauthorized.**
  - **Carried-forward limits (stated, not solved):** (i) **no LOST case exercised on live** — the first real losing resolution is its first live exercise; inspect it when it books. (ii) **cap_suspect (Ruling H): 6 whales returned exactly 100 `/positions`** (un-paginated page cap) → captures for high-volume whales are a **lower bound** (a trackable line past #100 is unseen); pre-existing shared-client limit, not a poller bug. (iii) **mlb slug/endDate mismatch** (`mlb-sf-atl-2026-06-18` vs gamma `endDate 2026-09-07`, same condition_id) — behaviour correct (adjudicator keys off condition_id); **open observation, do not chase** — something upstream reuses/mislabels a slug.
...
> - **Phase 2 — PER-CATEGORY CONTENT (BUILT 2026-08-28, branch `prediction-markets-stage2-phase2-2026-08-28`; record `STAGE2_PHASE2_PERCATEGORY_2026-08-28.md`; box-scratch PENDING, NOT deployed).** Filled the Watchlist (paper: live OPEN count [R6] + closed perf + `stale`/`void` beside closed + Analyze WIRED + Demote/Promote DISABLED) and Prospects (completed ranking via `query_scoreboard` filtered to candidates + caveats + Promote-to-watchlist DISABLED + honest-empty "no search yet") sections; added the pinned-whale PAPER detail `GET /watchlist/{w}/{cat}` (paper reader in `positions.py`; `paper.py` untouched); the completed/prospect drill reuses the existing `/whale` detail + `pm_position_rows.html`. BASIS test asserts DISPLAYED values (paper vs completed). `query_scoreboard`/`stats.py`/`pm_macros.html` untouched (legacy `/farm` byte-identical). Sanity finding: `/farm` "N polled, M with open" counts active PAIRS while "118 open" counts all TRADES incl. deactivated pairs — reconcilable, NOT a bug. No route removal.
> - **Phase 3 — RETIRE + REPOINT (BUILT 2026-08-28, branch `prediction-markets-stage2-phase3-2026-08-28`; record `STAGE2_PHASE3_REPOINT_RETIRE_2026-08-28.md`; box-scratch PENDING, NOT deployed). All 6 checklist items DONE:** repointed /dashboard→/, /farm-league→/farm, /farm-league/{cat}→/farm/{cat}; retired scoreboard_page+scoreboard_table+farm_page+farm_list+their loaders+ALLOWED_ROUTINES (query_scoreboard the FUNCTION stays as the ranker); NO aliases (temp paths 404); fixed every nav/breadcrumb link incl the whale pages' stale `← scoreboard` crumb; base-consolidation MINIMAL = whale pages → pm_shell + `pm_base.html` DELETED (ONE shell); deleted pm_scoreboard/pm_scoreboard_table/pm_farm/pm_farm_lists/pm_base. 9 modified + 5 deleted; rung ladder custody covers modified/DELETED (rollback restores deleted pages). Scoreboard caveat-render tests deferred to Stage 4 (Prospects empty till Search). **The LOAD-BEARING deliverable below (the 6 items) is now satisfied.**
>
