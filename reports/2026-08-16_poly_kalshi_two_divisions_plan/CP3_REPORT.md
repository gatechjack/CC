...
1. **RESOLVED (folded in per operator go) — OPEN badge count.** `_query_pm_pending_count` now has the parallel poly_kalshi `COUNT(*)` block with the **SAME WHERE** as the open-trades branch (kind/status/action/division + `order_id` LEFT-JOIN gate), so badge == list by construction. Single writer of `kind='poly_kalshi_order'` (`poly_kalshi_executor.py:368`, actor==division) means the division predicate fully scopes it; the arb 6-actor COUNT is untouched. **Proven:** on the 3 real fills, OPEN list = 3 and badge = 3 (agree, not 0); when one resolves both drop to 2 in lockstep. Failure count unchanged (10 current == 10 pristine).
