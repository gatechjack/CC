# pk_add_fed_paper.ps1 -- add the 2 Fed-scout whales (Kickstand7 + pako) to the PCT PAPER roster
...
# Run: powershell -ep bypass -f .\pk_add_fed_paper.ps1 -Apply -Pin
...
ADD=[
 {"wallet":"0xd1acd3925d895de9aec98ff95f3a30c5279d08d5","user_name":"Kickstand7","category":"FED","tier":1,"source":"fed_scout_2026-08-21","note":"win_px0.77 called-contested edge-profile n3-thin huge-bets; observe fwd"},
 {"wallet":"0x71edffd0d70a1da823ff07a3c6fc81457294d338","user_name":"pako","category":"FED","tier":1,"source":"fed_scout_2026-08-21","note":"+629k/+34pctROI n8 win_px0.86 biggest-net partial/truncated; observe fwd"},
]
...
print("INVARIANT live-clash:", clash if clash else "NONE (live INTERSECT fed == empty, OK)")
if clash:
...
print("both FED present in selected_whales:", ok)
...
$tf  = Join-Path $env:TEMP 'pk_addfed_chunk.sh'
$size = 20000; $first = $true
for ($i = 0; $i -lt $b64.Length; $i += $size) {
    $chunk = $b64.Substring($i, [Math]::Min($size, $b64.Length - $i))
    $op = if ($first) { '>' } else { '>>' }
    [IO.File]::WriteAllText($tf, "printf %s '$chunk' $op /tmp/pk_addfed.b64`n", $enc)
...
[IO.File]::WriteAllText($tf, "base64 -d /tmp/pk_addfed.b64 | bash`n", $enc)
Write-Host "== ADD FED WHALES TO PCT PAPER ROSTER (mode=$mode pin=$pinv) =="
