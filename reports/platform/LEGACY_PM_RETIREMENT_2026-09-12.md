...
| 5-6 (=Jack's "Phase 6") | Code removal — **Option 1 (deploy main.py graft ONLY; defer ALL file deletions + web surgery)** per Jack's ruling. Deploy = 3 things: main.py loop graft + poly_kalshi `auto_execute:false` + 4 timer units removed. NO file deletions / web routes / test removal — the ~30-file DELETE set is a TRANSITIVE CLOSURE (2 survivor-coupled keepers found: `_weather_math`←path_logger, `kalshi_crypto_v2_observer`←PID 679) deferred to the untangle pass. | **BUILT + PROVEN LOCAL (session 6) — graft re-proven at push time (survivors unchanged, legacy=0, dangling=0, py_compile OK; md5 `c15b4de6…`); §16.7 all 8 shared files UNCHANGED; prod-live tip==`fcbcd4a7`. NOT pushed/deployed/restarted.** Revival SHA `fcbcd4a7`. | **YES ×1 (Jack)** | PM stays armed; gate = PM PLACING within 20 min (§18.5) | forward-revert FF; box backups per file |
...
### 18.6 STATUS (2026-09-13, session 6): Option-1 build **COMPLETE + PROVEN local**. `main.py` graft + `poly_kalshi auto_execute:false` staged on `legpm-code-removal-2026-09-13` (off prod-live `fcbcd4a7`), re-proven at push time (see intro). HANDED TO JACK, in order: **(1)** prod-live FF push of the deploy commit; then **(2)** box graft-deploy runner (re-prove md5 `c15b4de6…` + counts on the DEPLOYED file, drift-gated); **(3)** `systemctl restart trading-corp`; **(4)** the 4-timer-unit removal runner (az-root); then the post-restart PLACING gate (18.5) and advance prod-live the same session. Nothing pushed / restarted / deleted / dropped by me. No file deletions, no web surgery — deferred to the untangle pass (18.3).
