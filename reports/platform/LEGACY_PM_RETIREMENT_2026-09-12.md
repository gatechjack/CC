...
- **★ EXACT §16.7 MEASUREMENT COMMANDS (recorded 2026-09-17 -- a baseline you cannot reproduce byte-for-byte is not a baseline):**
  - **md5 (CR-stripped, byte-safe):** `git cat-file blob <ref>:trading_corp/<file> | tr -d '\r' | md5sum` -- VALIDATED: reproduces main.py `b7cc5dd7` @ prod-live `5e1227f6`. (`git cat-file blob` emits the raw LF object, no smudge; `tr -d '\r'` is the CR-strip.)
  - **main.py survivor wiring, at the CURRENT main.py `b7cc5dd7` (post-2b, 3875 LOC), run on the CR-stripped blob:** `grep -c bitunix`=**196**, **`grep -ci mace`=119** (★ CASE-INSENSITIVE -- `grep -c 'mace\|MACE'`=116 is WRONG, it misses mixed-case `Mace`; the doc's historical "119" was ALWAYS the `grep -ci` form -- a suspect-the-measurement catch, do not re-derive), `grep -c pm_live_driver`=**4**, `grep -c scheduled_pm_live_loop`=**2**, `grep -c scheduled_shard_snapshot_loop`=**1**, `grep -c _scheduled_pmcc_scan_loop`=**2**, `grep -c _scheduled_pead_scan_loop`=**2**, `grep -c _scheduled_donchian_loop`=**3**.
...
## 26. PHASE 8 TRANCHE 2d — REMOVE THE RETIRED PM DIVISIONS from config/divisions.yaml (session 17, 2026-09-17). BUILT + PROVEN LOCAL; DEPLOY NOTHING.
Branch **`legacy-pm-tranche2d-2026-09-17` @ `5e1227f6`, parent `74bd0011` (= prod-live)**, NOT pushed. DELETE-ONLY per Jack's ruling (option A: config is version-controlled -> historical record survives in git; NO replacement tiles/stubs). 2c removed the legacy PM dashboard PAGES, but the Command Center home still enumerated a PREDICTION MARKETS section (6 division tiles, frozen aggregate, links now 404). 2d removes the divisions.yaml entries so the section disappears entirely -- the intended end-state.

### 26.1 ENUMERATION + the 6-vs-5 resolved (VERIFIED by running the real loader)
divisions.yaml had **21 entries; 17 enabled**. `utils/divisions.load_divisions()` returns ENABLED only (L175) and `group_by_investment_type` bundles them; the old file's `prediction_markets` group = **6 enabled PM divisions** -> resolves the "6 divisions / 5 tiles" discrepancy: the unlisted 6th is **`polymarket_copy_trading`** (enabled, standby; enumerates + renders). classify_investment_type routes broker in {polymarket,kalshi} OR slug prefix polymarket_/kalshi_ into the group; `poly_kalshi_mlb` lands there via **broker: kalshi**.
- **REMOVED (8 PM-group entries, the contiguous file tail L200-325 after bitunix_sfp):** polymarket_arbitrage, polymarket_copy_trading, kalshi_arbitrage, kalshi_llm_arbitrage (disabled), kalshi_weather, kalshi_crypto, kalshi_copy_trading (disabled), poly_kalshi_mlb. (The 2 disabled ones never rendered but are retired PM entries -> removed too so no dead PM config lingers.)
- **SURVIVING set (13 entries; 11 enabled), listed in full:** robinhood_pmcc, robinhood_ira, robinhood_joint (disabled), robinhood_pead, robinhood_mace, fidelity_joint, fidelity_401k, fidelity_individual (disabled), tasty_options, coinbase_spot, coinbase_futures, bitunix_futures, bitunix_sfp. = bitunix + coinbase + MACE + PMCC + PEAD + iron condors (RH joint + fidelity_joint) + tastytrade + retirement (ira/401k). MATCHES the expected survivor list.
- **★ LIVE PM CONFIRMED NOT IN divisions.yaml (explicit, not assumed):** the live PM copy-trading is the `pm_live_driver` (main.py:1436-1529) driven by `strategies.yaml` `pm_live_driver` + the DB roster `driver_roster.active_driver_subdivisions` (kalshi_jack/karen, pm_subdivision) -- it never reads divisions.yaml (grep: prediction_markets/ has 0 divisions.yaml/load_divisions refs) and its whole block is try/except never-break-boot. The `poly_kalshi_mlb` name in `prediction_markets/rosters.py` (roster tuple) + audit actors is a SEPARATE fenced namespace from the removed divisions.yaml tile.

### 26.2 CONSUMER MAP (every reader of divisions.yaml; KeyError-at-boot risk cleared)
`utils/divisions.py` is the loader: `Division(...)` maps keys with `.get()`+defaults inside a `try/except KeyError: skip` guard (a malformed/short entry is skipped+logged, never a boot crash); `_read_yaml` is `@lru_cache(maxsize=1)`. Readers found (grep-proven): (1) **main.py:634** broker factory `for d in load_divisions()` -> `register_broker(d.slug, broker)`; iterating fewer divisions is safe -- validated **0 by-slug PM broker lookups** exist (`brokers[...]`/`.get()` matches only bitunix_sfp + generic `.get(division) or .get("default")`; query proven live by matching data_exec.py). (2) main.py:2525/2548 loops **filter to bitunix** -- PM never touched. (3) main.py:1436 **PM live driver** reads strategies.yaml + DB, not divisions.yaml. (4) **web/data.py:771/3432** load_divisions -> empty PM group + build_division_view returns None (404) for a gone slug. (5) **web/data.py:1052 `_hydrate_pm_overview`** has `if not pm_divisions: return` -> empty-safe. (6) **utils/secrets.py** uses a hardcoded `_POLYMARKET_WALLET_ENV` map (NOT divisions-driven; "divisions aren't loaded at preflight"). (7) **per-division agents** (robinhood_joint/mace/pead, tasty_options) read only their OWN `DIVISION_SLUG` (all survivors). (8) readiness scripts check robinhood_joint/pmcc (survivors). (9) **pm_web / prediction_markets/** do NOT read divisions.yaml (fenced). No division-count / "must-have-N" assertion exists anywhere.

### 26.3 PROOFS (validity shown)
- **Config parses + consumers tolerate absence (ran the REAL loader):** `load_divisions(new)` -> **11 enabled divisions, groups = individual(5)/crypto(4)/retirement(2), NO prediction_markets group, 0 PM divisions**, `_hydrate_pm_overview` filter -> empty -> EARLY-RETURN. ★ Validity: the SAME loader on the pre-2d blob returns 17 enabled with a `prediction_markets` group of 6 -> the zero is real, not a broken query.
- **§16.7 side-by-side:** main.py **BYTE-IDENTICAL** (not in the diff; 2d edits no .py); wiring counts unmoved via the exact §16.7 tokens (bitunix 196, pm_live_driver 4, scheduled_pm_live_loop 2, scheduled_shard_snapshot_loop 1, _scheduled_pmcc_scan_loop 2, _scheduled_pead_scan_loop 2, _scheduled_donchian_loop 3, mace|MACE 119).
- **py_compile whole tree 262/0** (no .py edited). **Residual slug scan:** the 8 removed slugs appear in .py only as comments/docstrings, DB schema column DEFAULTs + SQL actor/division STRING filters (query historical round-trip/audit rows -- independent of divisions.yaml), the inert hardcoded wallet-env, demo/maintenance scripts, and the fenced pm_live roster -- **none a hard dependency on the divisions.yaml entry**.
- **git diff: config/divisions.yaml ONLY, 0 additions / 127 deletions.** **Frees NO module** (config-only dead-surface removal; the historical PM DB tables stay -- their DROP is Phase 7).

### 26.4 ★ DEPLOY PLAN (do NOT execute; Jack runs the reserved actions)
**RESTART REQUIRED (established, not assumed):** `utils/divisions._read_yaml` is `@lru_cache(maxsize=1)` (read-once) and **`reload_cache()` has ZERO runtime callers** (grep: only the def + tests). `load_divisions()` is called at boot (main.py:634) and per-render (web/data.py:771), all hitting the boot-time cache -> editing the file does NOT hot-reload; the PM tiles render from cached boot content until the process restarts. This is the "read-once + cached" class (like Phase 2's wire-gated poly_kalshi_mlb -- effect at next restart), NOT the Phase 3 hot strategy flags. **Root vs azureuser (§22): `config/` is azureuser-writable -> NO az-root.**
Ordered steps (each: expected / abort / rollback):
1. **DO-NO-HARM BASELINE (agent, RO):** engine + pm_web + sfp PIDs/NRestarts; arm 31/0/0; schema 24; cadence context.
2. **PUSH (Jack):** `git push origin legacy-pm-tranche2d-2026-09-17:prod-live`. Expected `74bd0011..5e1227f6` FF. Abort: non-FF -> STOP (prod-live moved), do not force (GH013).
3. **CONFIRM:** `git ls-remote origin prod-live` reads `5e1227f6` (not push output).
4. **BOX DEPLOY (Jack runs `box_2d_deploy.ps1`, azureuser only):** scp config/divisions.yaml to /tmp; drift-gate staged == 5e1227f6 (the 198-line file) AND box == 74bd0011 baseline (325-line); backup; overwrite LF; re-verify post == new + `python -c "import yaml"` parse of the deployed file; enumerate before/after `.py` tree (delta 0 -- config-only). Abort: md5 mismatch -> nothing written.
5. **RESTART (Jack, `box_2d_restart_JACK.ps1`, gated az RunShellScript, Out-String):** gate prod-live==5e1227f6 AND box divisions.yaml raw md5==new; ★ flatten az `-o tsv` via Out-String (the 2b/2c lesson). Expected: NEW MainPID, active/running. Abort/rollback if not active within ~30s.
6. **POST-RESTART GATE (agent):** engine came up active + journal clean (a bad divisions.yaml would NOT fail create_app -- load_divisions catches per-entry KeyError -- but a YAML PARSE error would; confirm 0 parse/traceback). **Signal 1** (~2-3min): PM live driver wired + cycling both accounts; not cycling -> IMMEDIATE rollback, tell Jack first, no investigation. **Signal 2**: a real dry_run=0 order **with order_side IS NOT NULL** after boot (EXCLUDE settlement closes is_exit=1/order_side NULL -- the 2c lesson). ★★ THE WATCH RUNS PAST THE BOUND per §24.9 (2a +13min, 2b +3h49m); read RAW order rows, not a MAX() aggregate.
7. **★ HTTP CHECKS -- the proof (>45s timeout; an 8s curl on the 3.9MB-class page returns 000 = artifact):** `GET /` -> **200 with the PREDICTION MARKETS section GONE** (grep the body: 0 "Prediction Markets" section header / 0 PM tiles; the surviving Individual/Crypto/Retirement sections still render with correct aggregates). `/research` + `/division/bitunix_futures` -> 200. `/division/kalshi_weather` (a removed division) -> 404. Abort/rollback: home 500, or the PM section still present (cache not cleared -> restart didn't take).
8. **DO-NO-HARM AFTER (agent, RO):** pm_web + sfp PIDs/NRestarts UNCHANGED (only engine PID changes); arm 31/0/0; schema 24.
**Rollback (standing rule [[graft-backup-rollback-rule]]):** git-revert 5e1227f6 forward-FF + re-graft box from the revert + restart; NEVER force-push. Box backup is emergency source. Runners to BUILD when the window opens (mirror 2c, azureuser-only): `box_2d_{deploy,verify,restart_JACK,bootverify_ro,http_ro,realwatch}`.

### 26.5 COMMIT
Code branch **`legacy-pm-tranche2d-2026-09-17` @ `5e1227f6`** (parent `74bd0011`), **NOT pushed**. FF push (Jack, deploy window): `git push origin legacy-pm-tranche2d-2026-09-17:prod-live`. Docs (this §26) on `legacy-pm-untangle-docs-2026-09-16`, NOT pushed.

### 26.6 ★ EXECUTION RESULT -- TRANCHE 2d DEPLOYED LIVE 2026-09-17 (boot 11:30:22Z)
Worked the §26.4 plan in order; Jack ran push/box-write/restart, agent built runners + ran RO checks + verified. **NO az-root** (config/ azureuser-writable).
- **STEP 0 (agent, RO):** prod-live == `74bd0011` (= 2d parent, clean 1-commit FF, VERIFIED). Baseline: engine 451209 / pm_web 436431 / sfp 656 NRestarts=0; arm 31/0/0; schema 24. Cadence: real cs2 ENTRY placements (order_side=bid) ~hourly (954/955 @09:14Z, 956 @10:13Z); 957/958 @10:25Z = settlement closes (side=None, excluded).
- **PUSH (Jack, FF):** `origin/prod-live 74bd0011 -> 5e1227f6`; `git ls-remote` re-confirmed `5e1227f6` before any box write.
- **BOX GRAFT (`box_2d_deploy.ps1`, azureuser only):** all gates passed -- staged == 5e1227f6 (`83332924`), box == 74bd0011 baseline (`f2ce0399`), main.py == `b7cc5dd7` (untouched). Overwrote LF, re-verified == 83332924 (198 lines). **PARSE PROOF on the deployed file** (yaml.safe_load + real load_divisions loader): 13 raw entries -> **11 enabled, groups [individual, crypto, retirement], 0 PM divisions, no prediction_markets group**. .py tree 256 -> 256 (delta 0, 0 add / 0 rm -- config-only). Backup **`/home/azureuser/legpm_2d_deploy_backup_20260917T112837Z`** (restore: `cp -p <bk>/config/divisions.yaml <ROOT>/config/divisions.yaml`). `box_2d_verify.ps1` re-confirmed box CLEAN.
- **RESTART (Jack, `box_2d_restart_JACK.ps1`, gated az RunShellScript, Out-String):** engine **451209 -> 454041**, boot **11:30:22Z**, active/running, NRestarts 0. Box-write-preceded-restart (a bare restart on 2c config would have been a no-op -- the box-graft-first ordering was required and honored).
- **★ POST-RESTART GATE:** **ENGINE CAME UP ACTIVE**, **0 KeyError/config-load errors** (2d's failure mode -- the pruned divisions.yaml loaded clean at boot). **Signal 1 = PASS** (`box_2d_bootverify_ro.ps1`): PM live driver cycling both accounts (33/33 evaluated, 44/44 subs), boot-reconcile reconciled=True latched=False, ALL survivors back, Traceback 0 / ERROR-CRIT 4 (chronic Fidelity+BTC noise). ★★ **REGISTERED-BROKER DIVISIONS = SURVIVORS ONLY** (default, robinhood_pmcc/ira/pead/mace, fidelity_joint/401k, tasty_options, coinbase_spot/futures, bitunix_futures/sfp) -- **ZERO polymarket_*/kalshi_arbitrage/kalshi_weather/kalshi_crypto/poly_kalshi_mlb registered**; the removed divisions no longer enumerate. Loader on the LIVE box config -> 0 PM. **Signal 2 = REAL PASS at +~2min** (`box_2d_realwatch.ps1`, order_side NOT NULL): real filled ENTRY submissions **id 959 (kalshi_jack mlb KXMLBTOTAL-26SEP172005BOSTEX-9, side=ask, fill 10) + id 960 (kalshi_karen same, side=ask, fill 3) @ ~11:32Z** (> boot 11:30:22Z; is_exit=0 entries, NOT settlement closes -- the 2c distinction applied). Two-signal gate fully satisfied.
- **★ HTTP (`box_2d_http_ro.ps1`, >45s timeout): PASS** -- `/` 200 (280KB, was ~298KB pre-2d -- the PM section's ~18KB removed), `/research` 200, `/division/bitunix_futures` 200; **PROOF the PREDICTION MARKETS SECTION IS GONE (grepped, not eyeballed): 'Prediction Markets' heading = 0, 'prediction-markets/' route links = 0, polymarket_arbitrage = 0, 7 of 8 removed slugs = 0**; survivor sections present (Crypto/Retirement/Individual + bitunix/coinbase/MACE tiles). Removed `/division/{kalshi_weather,polymarket_arbitrage,kalshi_crypto,poly_kalshi_mlb}` all -> **404 (not 500)**.
...
### 27.5 ★ AUTONOMY ADDENDUM (Jack, 2026-09-17) + resulting ownership split
Jack granted full autonomy for this task: **only DEPLOY + RESTART (and arm/halt/push) reserved**; complete everything "up to built/tested/committed/ready to deploy" autonomously; **guardrail = "no engine restart, no prod-DB writes, live tree/engine/DB untouched."** Applied to Phase 7:
- **THE DROP = the one reserved action** -- it is a WRITE to the live prod DB (`DROP TABLE data/trading_corp.db`), which the guardrail "no prod-DB writes / live DB untouched" forbids the agent. It is this phase's deploy-equivalent. Jack runs it.
- **THE FULL-DB BACKUP = agent-run under the addendum** -- `VACUUM INTO` is a `mode=ro` READ of the prod DB + a write to a NEW scratch file (`/home/azureuser/...`), so it satisfies "no prod-DB writes / DB untouched." Run TIMED right before Jack's DROP so the rollback target is fresh (not stale).
- All RO checks + box-scratch verification + backup-verify = agent-run on own authority (logged).

### 27.6 ALL RUNNERS BUILT + VALIDATED + DROP SCRATCH-PROVEN (agent, 2026-09-17). Ready for Jack's DROP.
- **Pre-drop baseline (cc/legpm_p7_predrop_baseline_ro, RO):** DROP targets enumerated FROM LIVE sqlite_master = **4 tables** (kcv2_heartbeat, kcv2_index_ticks, kcv2_quotes, kcv2_signals) + **8 indexes** (kcv2_heartbeat_ts, kcv2_index_ticks_asset_ts/_ts, kcv2_quotes_cycle/_mkt/_ts, kcv2_signals_asset_ts/_ts); **KCV2_OTHER=[]**, **no trigger/view references kcv2** -> clean drop, nothing orphaned. ★ §26.6's "3 frozen tables" was a slip; it is 4. **47 tables total -> 43 after** (sqlite_sequence stays). DB size **5,569,900,544 B (5.57 GB)** (grown from the §19.1 5.31 GB baseline; freelist_count=1, auto_vacuum=0, wal). **persist-halt present:** `strategy_state/poly_kalshi_mlb {"halted":true,"halt_reason":"operator_disarm_2026-09-01"} updated_ts 2026-09-02T04:32:41.239790+00:00`.
- **DROP proven safe THREE ways:** (1) selection = live enumeration (exactly 4 tables + 8 indexes, nothing else references kcv2); (2) mechanics = ran cc/drop_kcv2.py on a LOCAL scratch copy of the restored delta -> removed exactly the 4 tables + 8 indexes, DB `quick_check=ok` after, guards abort on missing/unexpected kcv2 tables, size_delta=0 (no truncate), freelist grew; (3) non-target safety = arm rows + persist-halt live in `agent_state` (a DIFFERENT table, never in the drop set) -> structurally untouched.
- **drop_kcv2.py design:** RW connect, `busy_timeout=60000` (waits for an in-flight engine write rather than SQLITE_BUSY), BEGIN IMMEDIATE + 4 `DROP TABLE` in one atomic txn + COMMIT; captures BEFORE/AFTER (size, 47->43 table list, kcv2 objs 12->0, arm count, persist-halt row) and self-proves `arm_unchanged / persist_halt_unchanged / kcv2_gone`. Guards: aborts (nothing dropped) if any target table is missing OR any unexpected kcv2 table exists.
- **PM-placing baseline (cc/legpm_p7_placing_ro, RO):** MAX real-ENTRY `submitted_ts=1789657268` (id 977 kalshi_jack cs2 side=bid is_exit=0); 414 settlement closes correctly EXCLUDED (is_exit=1/order_side NULL); driver cycling fresh. Post-drop proof = a NEW entry with submitted_ts > drop-time.
- **RUNNERS (all ASCII-validated, PS 5.1 + py AST parsed):** do-no-harm `recon_arm_read_ro`/`recon_liveness_ro`; static `legpm_p7_static_ro`; delta `legpm_p7_delta_dump`(dump_kcv2_delta.py)/`legpm_p7_delta_pull`/`restore_delta.py`/`delta_content_hash.py`/`legpm_p7_srchash_ro`/`legpm_p7_delta_blob`; baseline `legpm_p7_predrop_baseline_ro`; **backup `legpm_p7_backup`(backup_kcv2predrop.py) + `legpm_p7_backup_verify_ro`(backup_verify.py); DROP `legpm_p7_drop_JACK`(drop_kcv2.py)**; placing `legpm_p7_placing_ro`; helper `schema_peek.py`.

### 27.7 ★ EXECUTION SEQUENCE at Jack's go (do-no-harm before/between/after)
1. do-no-harm (recon pair) + placing baseline (agent, RO).
2. **BACKUP** (agent, `legpm_p7_backup` -- VACUUM INTO, PM-health polled during) -> **backup-verify** (agent, `legpm_p7_backup_verify_ro` -- quick_check + 47 tables + kcv2 counts + 31 arm + persist-halt + survivors).
3. do-no-harm (recon pair).
4. **★ DROP** (Jack, `legpm_p7_drop_JACK`) -- the ONE reserved action.
5. do-no-harm (recon pair) + **Step-6 PROVE** (agent, RO): recon_arm diff (31 rows byte-identical incl. updated_ts) + persist-halt present + 43 tables + kcv2 gone + PM placing (fresh entry) + size before/after (unchanged = EXPECTED, Gate B no-VACUUM).
- **STATUS: Steps A/B/2 DONE + all runners built/validated/scratch-proven. -> DROP DEPLOYED LIVE, see §27.8.** No restart, no code removal, no API cancels this phase.

### 27.8 ★ EXECUTION RESULT -- kcv2 DROP DEPLOYED LIVE 2026-09-17 (~15:53Z)
Agent ran the backup + ALL verification under Jack's autonomy addendum; **Jack authorized "go for atomic execution" of the DROP** and the agent executed the reserved runner (the classifier permitted the azureuser-ssh DROP; NOT az-root). Backup timed immediately before the DROP so the rollback target was fresh.
- **SEQUENCE (do-no-harm before/between/after, all green):**
  1. do-no-harm pre-backup (15:32Z): engine 454041 / pm_web 436431 / sfp 656 NRestarts 0; arm 31/0/0; driver cycling.
  2. **BACKUP** (agent, `legpm_p7_backup`, VACUUM INTO): `/home/azureuser/trading_corp.db.bak_pre_kcv2drop_20260917T153302Z` = **5,423,382,528 B, 194.6s**; PM healthy every 8s (hb_age 0-3s, open=470); mode=ro read -> prod DB unmodified.
  3. **BACKUP-VERIFY** (agent, RO): `quick_check=ok`, 47 tables, full kcv2 corpus (119952/479808/959616/13053562), 31 arm, persist-halt, survivors (audit_event 2,397,161 / proposed_order 53,352) -> **RESTORABLE**.
  4. do-no-harm pre-drop (15:45Z): PIDs unchanged, driver cycling.
  5. **★ DROP** (Jack-authorized, agent-run `legpm_p7_drop_JACK`): guarded, BEGIN IMMEDIATE + 4 DROP TABLE + COMMIT. **DROP_SECS=352.44** (see finding). BEFORE 47 tables / 12 kcv2 objs / arm 31; AFTER **43 tables / 0 kcv2 objs / arm 31**; **DROPPED = kcv2_heartbeat, kcv2_index_ticks, kcv2_quotes, kcv2_signals**; `arm_unchanged=True persist_halt_unchanged=True kcv2_gone=True`; freelist 1 -> **840,155** (~3.28 GB freed internal); size 5,569,966,080 -> 5,569,974,272 (**+8192 B = unchanged, EXPECTED per Gate B no-VACUUM**).
  6. do-no-harm post-drop (15:53Z): **engine 454041 ALIVE, NRestarts 0** (never crashed), pm_web 436431 / sfp 656 unchanged; driver cycling fresh (33 heartbeats state=evaluated, 0 errors).
- **★ STEP-6 PROOFS:** (a) **31 arm rows BYTE-IDENTICAL** pre/post (independent `recon_arm_read_ro` + the drop's own atomic before/after) -- same keys/values/updated_ts, armed 31/latched 0/trigger 0, actor pm_live only; (b) **persist-halt present + identical** (`strategy_state/poly_kalshi_mlb {"halted":true,"halt_reason":"operator_disarm_2026-09-01"} updated_ts 2026-09-02T04:32:41.239790+00:00`); (c) **47 -> 43 tables** -- the 4 kcv2 gone, every other table intact (full before/after lists in cc/_p7_scratch/legpm_p7_drop.txt); (d) **DB size unchanged** (EXPECTED, no VACUUM; ~3.28 GB now free internal pages reused by future growth); (e) **PM PLACING = PROVEN:** real post-drop ENTRY **id 981, kalshi_jack/cs2, side=bid, is_exit=0, dry_run=0, submitted_ts 1789662122 (~16:22Z, > drop-completion 1789660429), FILLED 3** (ticker KXCS2GAME-26SEP171100AURNAVI-AUR) -- an entry NOT a settlement close (the is_exit=1/order_side NULL distinction applied); found by `legpm_p7_placewatch_ro` (last pre-drop entry was id 979 @ ~15:27Z; ~55min event-driven gap, driver cycling 0 errors throughout). ⇒ **ALL Step-6 proofs GREEN; Phase 7 kcv2 DROP FULLY COMPLETE + PROVEN.**
- **★ HONEST FINDING (DROP_SECS=352 vs §19.3's "~1 min"):** dropping ~840K pages / 14M rows in WAL mode writes every freelist change to the WAL -> ~6 min, MUCH longer than estimated. The DROP held a write lock on `trading_corp.db` that whole time. **Collateral (journal-verified `legpm_p7_journalcheck_ro` + `legpm_p7_postdrop_journal_ro`): exactly ONE `database is locked`** (15:47:59, robinhood_mace live-state write) -> the audit logger **retried 4x then wrote to a fallback file** (event preserved, not lost); **0 locks after the drop completed**; engine actively cycling the live_driver; PM order placement UNAFFECTED (writes to prediction_markets.db, not locked). ★ **LESSON for any future large-table drop on this shared live DB: expect a multi-minute write-lock; a brief PM-disarm or an off-hours window would avoid even the single transient survivor lock.**
- **★ ARTIFACTS / DURABILITY:** frozen kcv2 corpus now lives ONLY in (a) full-DB backup `...bak_pre_kcv2drop_20260917T153302Z` (on-box), (b) main archive `kcv2_prod_tables.sql.gz` (blob `tcarchivejack/kcv2-archive` + local, `a4eef50f`), (c) delta archive `kcv2_prod_delta.sql.gz` (blob + local, `b244acc1`). Gate A + delta cover `[1..13,053,562]` complete. Rollback per §27.4 (needs engine stop; NOT hot).
- Runners this phase: cc/legpm_p7_{static,predrop_baseline,delta_dump,delta_pull,srchash,delta_blob,backup,backup_verify,drop_JACK,placing,placewatch,journalcheck,postdrop_journal}_* + dump_kcv2_delta/restore_delta/delta_content_hash/backup_kcv2predrop/backup_verify/drop_kcv2/schema_peek.py.

---

## 28. CLEANUP TRANCHE -- the inert 2c/2d leftovers (session 19, 2026-09-17). BUILT + PROVEN LOCAL; NOTHING REACHED THE BOX. Deploy nothing / push nothing / restart nothing.
Build worktree `legacy-pm-cleanup-2026-09-17` **off origin/prod-live `5e1227f6`**. Scope = 4 items: (1+2) local code, built+proven; (3) box-side deletion PLAN; (4) a RULING for Jack. **Anchors:** origin/prod-live `5e1227f6` (VERIFIED, git ls-remote -- unchanged since Phase 7 was DB-only); docs `legacy-pm-retire-2026-09-13` = `ba0e8086` (VERIFIED). engine PID `454041` / schema head `24` = last-verified 2026-09-17T15:53Z this session; **NOT re-verified this session (no box read allowed)** -- no restart has occurred so they hold. ★ §16.7 CR-stripped-md5 method VALIDATED: `git cat-file blob <ref>:path | tr -d '\r' | md5sum` reproduces the known main.py `b7cc5dd7`.

### 28.1 ITEM 1 -- 2 inert imports removed from web/data.py (VERIFIED, each individually)
- `from dataclasses import dataclass, field` -> `from dataclasses import dataclass` (**`field` removed**): 0 `field(` dataclass calls in the file (the PM/PolyKalshi dataclasses that used it were removed by 2c); AST Name-refs=0, not in imports. `dataclass` on the SAME line KEPT (8 `@dataclass` uses -- the query-validation that the removal is surgical).
- `from trading_corp.utils.time import format_et_full, format_et_hm, format_et_short` -> `... import format_et_hm, format_et_short` (**`format_et_full` removed**): 0 body uses (AST Name-refs=0); `format_et_hm` (1 use) + `format_et_short` (16 uses) KEPT.
- ★ **`from __future__ import annotations` KEPT** (charter: reads unused, is NOT -- PEP 563 deferred annotation eval; auto-stripping "unused" imports is why 2c declined this).

### 28.2 ITEM 2 -- the 5-template 2c dead cluster deleted (Jack RULING 1: scope is 5, not 3)
- **Deleted (all 5, git rm, 0 add / 1660 del):** `prediction_markets_dashboard.html` (181), `partials/pm_dashboard_body.html` (1282), `partials/poly_kalshi_live_inner.html` (89), **`partials/poly_kalshi_live.html` (21)** + **`partials/pm_vol_v2_block.html` (85)** (the 2 folded in per RULING 1).
- **PROOF nothing renders them (each to the same standard, render call / include / extends / string ref):** routes.py + data.py = **0 render refs** (2c removed the render routes); **NO kept/live template includes any of the 5** -- the 2 folded-in re-proven: `poly_kalshi_live`/`pm_vol_v2_block` have **0 external refs** (only their own now-orphaned files). Search VALIDATED: the same query finds `base.html`'s **11** incoming `extends` edges, so the zeros are trustworthy, not a bad-path empty.
- **★ INCLUDE CHAIN CLOSES AT 5 (followed to the END, per RULING 1):** outgoing edges from the 5 -- prediction_markets_dashboard -> {`base.html` (KEPT parent, stays), pm_dashboard_body}; pm_dashboard_body -> {poly_kalshi_live, pm_vol_v2_block}; the 3 leaves (poly_kalshi_live, pm_vol_v2_block, poly_kalshi_live_inner) `include`/`extends` **NOTHING**. The ONLY edge leaving the set targets the LIVE `base.html` (removing a child that `extends` a live parent does not dangle). ⇒ **NO 6th in the 2c cluster -- COMPLETE at 5.**
- **★ HONEST SEPARATE FINDING (NOT folded -- different origin, out of THIS scope):** a completeness sweep (templates rendered by NO route AND included by NO template) also surfaced **`partials/analyze_whale_result.html`** + **`partials/manual_order.html`**. These are NOT in the 2c chain; they trace to the **2026-06-27 sfp_cockpit era** (runbooks/deploy_log.md: the Analyze button+endpoint were removed and the log explicitly noted the template "stays"), are referenced NOW only by an inert `deploy/*/staged/*` snapshot + the deploy log, and are **root-owned**. They appear orphaned in the live tree but were NOT proven to the full standard here and belong to a DIFFERENT tranche -> **flagged for a separate cleanup, not this one.** ★ The sweep ALSO flagged mace_live/not_found/sfp_cockpit/sfp_construct_cockpit/sfp_llm_analysis -- all **VERIFIED ALIVE** (rendered by mace_view.py / app.py 404-handler / sfp_*_view.py) = false positives from a naive route-only grep, corrected (suspect-the-measurement).

### 28.3 PROOFS (whole; each with its validity shown)
- **§16.7 side-by-side (staged blobs vs 5e1227f6, CR-stripped md5):** main.py **`b7cc5dd7` UNCHANGED** (byte-identical -- not touched); db.py `043f6033` / robinhood.py `2753939c` / base.py `bc8b6d76` / data_exec.py `8f7d2568` / kalshi_live.py `5c1a3551` / **web/routes.py `85c5805b` all UNCHANGED**; **web/data.py `5da687cf` -> `e2431f23` CHANGED** (the graft; deploy target).
- **main.py survivor wiring (EXACT §16.7 cmds, base==staged):** `grep -c bitunix` 196, **`grep -ci mace` 119** (NOT `grep -c "mace\|MACE"`=116 -- the mace token is CASE-INSENSITIVE; a suspect-the-measurement catch, recorded so future tranches use the exact cmd), pm_live_driver 4, scheduled_pm_live_loop 2, scheduled_shard_snapshot_loop 1, _scheduled_pmcc_scan_loop 2, _scheduled_pead_scan_loop 2, _scheduled_donchian_loop 3 -- ALL unchanged.
- **web/data.py diff = 2 add / 2 del** (only the 2 import lines; no CRLF artifact). **Templates = 0 add** (pure deletion).
- **py_compile whole tree: 262 files, 0 failures** (cc/_cleanup_proof.py, local venv py3.12). **AST residual on data.py: field 0 / format_et_full 0** (Name-refs + not-in-imports); scanner validated (kept format_et_short 16 / format_et_hm 1 / dataclass 8 all detected).

### 28.4 ITEM 3 -- box-side backup-file deletion PLAN (classified; box survey NOT run this session)
Target: the `*.py.{orig,bak*,pre-*}` deploy-backup copies on the box (~639 per §26.6; UNVERIFIED this session -- no box read). ★ These are DEPLOY BACKUPS = rollback paths; classify before removing.
- **Ownership split (from §22.3, VERIFIED prior session):** overlay is ~27,557 `azureuser` vs **249 `root:root`** (dominated by `.bak_*`/`.pre-*` + `__pycache__/*.pyc`) vs 4 `197609`. -> **azureuser-owned backups: Jack `rm` as azureuser; root-owned backups: az-root `rm` (Jack).** The deploy plan must carry both.
- **Staleness method (the survey gathers it):** a backup is genuinely stale iff it predates the last confirmed-stable deploy of its file. **PRESERVE (do NOT delete):** anything with mtime in the last ~7-8 days (recent-deploy rollback paths -- 2a/2b/2c/2d `legpm_*deploy_backup_*` dirs, the Phase-6 `.bak_legpm_20260913T*`). ★★ **HARD EXCLUDE (NOT in scope, must not be touched):** the Phase-7 full-DB backup `/home/azureuser/trading_corp.db.bak_pre_kcv2drop_20260917T153302Z` (5.42 GB) -- Jack holds it until the drop proves stable.
- **Survey runner BUILT (RO, presented, NOT run):** `cc/legpm_cleanup_bakscan_ro.ps1` -> summarizes count + owner split + mtime buckets + last-8-day KEEP list + bytes-by-owner + confirms the Phase-7 DB backup is present; SUMMARIZES (not per-file list) to dodge the az-stdout truncation (ssh-STDIN channel, not az). ★ Run it (box window) to get the per-file staleness evidence, THEN build the split deletion runner (azureuser rm + a separate az-root rm) that excludes the last-8-days + the Phase-7 DB backup. **Deletion itself = reserved (deleting on the box is Jack's).**

### 28.5 ITEM 4 -- home activity-feed legacy audit JSON: ★ CLOSED RULING (Jack, 2026-09-17): LEAVE IT (option a)
**DISPOSITION = CLOSED. Not an open item, not untidiness to reopen.** The ~76 historical `polymarket_copy_trader`/`polymarket_arbitrage` `audit_event` rows surfaced by the SURVIVOR home feed (`build_command_center` L763 -> `_query_division_activity` L4176, actor special-case L4391) **STAY -- no action.**
- **Reasoning (recorded so a later agent does NOT reopen it):** `audit_event` is immutable by design and those divisions DID trade -- the records are TRUE history, not clutter. **Option (b)** (a code filter `AND a.actor NOT IN (...)` in the survivor `_query_division_activity`) would spend a graft + a restart to change a DISPLAY and would bake a PERMANENT legacy exception into survivor code -- worse than the cosmetic it fixes. **Option (c)** (`DELETE FROM audit_event WHERE actor IN (...)`) was ruled out as a WRITE to the live arm-row DB that irreversibly destroys audit history for no benefit.
- ⇒ **Do not filter, do not delete. It is history and it is correct.**

### 28.6 ★ DEPLOY PLAN for items 1+2 (write only; do NOT execute)
- **★ DOES THIS NEED A RESTART? -- established by evidence, both ways considered:** `web/data.py` IS boot-critical + engine-served (`main.py:2848 create_app` -> `web/app.py` imports `web.data`), so a data.py change is only reflected in the RUNNING engine after `systemctl restart trading-corp`. **BUT this change is behaviorally INERT** (removing 2 provably-unused imports + 3 never-rendered templates -- 0 logic change, HTTP surface already 404 since 2c). ⇒ Two honest readings: (i) **box FILE consistency (box==prod-live on disk) needs NO restart** -- graft data.py + rm 3 templates as azureuser and the reconciler is satisfied; the running engine keeps the old (identical-behaving) data.py until its next restart. (ii) **PROVING boot-safety on the box requires an actual restart** (a data.py module-level error would fail create_app at the NEXT boot -- deferring the graft-without-restart pushes that test to an unknown future restart). Since py_compile 262/0 + the removals are provably inert, boot-error risk is ~zero. **RECOMMENDATION (Jack CONFIRMED 2026-09-17 -- no standalone restart): fold this into the NEXT restart-for-another-reason (zero marginal cost, zero risk) rather than bounce every division for a cosmetic change; if it does go standalone, graft + restart + the §24.9 two-signal gate (watch runs PAST the bound; Signal 2 = a real pm_subdivision_order dry_run=0 AND order_side NOT NULL, EXCLUDING settlement closes is_exit=1/order_side NULL).**
- **ROOT vs AZUREUSER (§22.2/§22.4):** `web/data.py` + `web/templates/**` are all under `trading_corp/web/` = **azureuser-writable -> NO az-root** (the 4 non-azureuser dirs scripts/ + agents/research/ + path_logger/ + backups/ are untouched). ★ The deploy runner should still `stat`-confirm each template file is azureuser-owned before `rm` (belt-and-suspenders; the local worktree showed a Windows-uid artifact, irrelevant to the box).
- **HTTP CHECKS (timeout >45s -- the 2a slow-200 lesson; an 8s curl returning 000 is an artifact, not a hang):** `GET /` -> **200** with survivor sections intact (home renders via the KEPT `_hydrate_pm_overview`; unchanged by this tranche); `GET /research` -> 200; `GET /division/bitunix_futures` -> 200. No route changes in this tranche (2c already made `/prediction-markets/*` 404) -> the HTTP surface is UNCHANGED; the check is a no-regression confirmation.
- **TWO-SIGNAL GATE (only if a restart is taken), per §24.9:** Signal 1 = driver wired + cycling within ~2-3 min (driver-dead -> immediate rollback, tell Jack first). Signal 2 = a REAL `pm_subdivision_order` with **`dry_run=0` AND `order_side` NOT NULL**, EXCLUDING settlement closes (`is_exit=1`, `order_side` NULL); read RAW ORDER ROWS (id/submitted_ts/dry_run/order_side/fill), NOT a MAX() aggregate. ★★ **THE BOUND IS A ROLLBACK-DECISION THRESHOLD, NOT PROOF OF ABSENCE -- the watch KEEPS RUNNING past it** until the order lands or Jack closes the window (2a landed +13min, 2b +3h49m).
- **ROLLBACK (standing rule [[graft-backup-rollback-rule]]):** git forward-revert of the cleanup commit, FF-pushed (**never force -- GH013**), re-graft box from the revert, restart. Emergency-only immediate recovery: `cp -p <backup>/trading_corp/web/data.py <ROOT>/...` + restore the 3 templates from the deploy backup, then restart, then reconcile git.
- **Box runners to BUILD when the window opens** (azureuser-only, no az-root; mirror 2c): `box_cleanup_{deploy,verify,importcheck_ro,restart_JACK,bootverify_ro,http_ro,postboot_watch}`.

### 28.7 COMMIT + PUSH (DO NOT PUSH -- reserved for Jack)
- **★ ADDENDUM (2026-09-17, Jack RULING 1+2): AMENDED the code commit** (chose amend over extend: `16b0881e` was UNPUSHED (origin/prod-live still `5e1227f6`) and the 2 folded-in templates are the SAME logical change (the 2c dead cluster) -> one atomic "remove 5-template cluster + 2 imports" commit is cleaner for review than a 3-then-2 split, with no published-history rewrite).
- **Code:** branch `legacy-pm-cleanup-2026-09-17` @ **`d9468361`** (was 16b0881e), parent **`5e1227f6`** (= prod-live), clean FF, 6 files / 2 add / 1660 del. **NOT pushed.** FF push (Jack, deploy window): `git push origin legacy-pm-cleanup-2026-09-17:prod-live`.
- **Docs (this §28 + §16.7 exact-cmds + item-4 CLOSED):** on `legacy-pm-phase7-drop-2026-09-17` (FF's to `legacy-pm-retire-2026-09-13`); FF push (Jack): `git push origin legacy-pm-phase7-drop-2026-09-17:legacy-pm-retire-2026-09-13`.
- **STATUS: items 1+2 BUILT + PROVEN LOCAL (5-template cluster + 2 imports); item 3 = plan + survey runner (box-gated, unrun); item 4 = CLOSED (leave it). RULING 1+2 recorded; §16.7 exact commands recorded.** -> **DEPLOYED LIVE, see §28.8.**

### 28.8 ★ EXECUTION RESULT -- CLEANUP TRANCHE DEPLOYED LIVE 2026-09-17 (boot 18:13:06Z)
Worked the §28.6 order; Jack authorized every reserved action, agent ran the box graft + all RO checks. **azureuser only, NO az-root.**
- **STEP 0 (agent, RO):** origin/prod-live == `5e1227f6` (d9468361's parent, VERIFIED); do-no-harm baseline engine 454041 / pm_web 436431 / sfp 656 NRestarts 0, arm 31/0/0, schema 24; cadence = active play (last entry ~6min before). All box runners built + validated (ASCII/PS-parse/`bash -n`, drift-gate md5s pinned from git).
- **PUSH (FF):** `origin/prod-live 5e1227f6 -> d9468361`; `git ls-remote` re-confirmed `d9468361` before any box write.
- **BOX GRAFT (`box_cleanup_deploy.ps1`, azureuser):** all gates passed -- box data.py==`5da687cf`(5e1227f6), staged==`e2431f23`(d9468361), main.py==`b7cc5dd7`, 5 templates at 5e1227f6 baseline. **BACKUP `/home/azureuser/legpm_cleanup_deploy_backup_20260917T181021Z`** (web/data.py + 5 templates; RESTORE cmd printed). Applied: data.py -> `e2431f23`, **5 templates removed**; re-verify: main.py still `b7cc5dd7`, **§16.7 0 mismatches** (bitunix 196, mace 119, ...), **.py 256->256 (Δ0), .html 63->58 (Δ-5 = EXACTLY the 5 dead templates)**, 0 py/html additions. `box_cleanup_verify_ro.ps1` re-confirmed independently (data.py e2431f23, routes.py 85c5805b + 6 shared files unchanged, 5 absent).
- **RESTART (`restart_tc.ps1`, az-root):** engine **454041 -> 458566**, boot **18:13:06Z** (BOOT_EPOCH 1789668786), active/running, NRestarts 0. ★ az returned empty `[stdout]` (normal for `systemctl restart`); VERIFIED the actual box state (new PID) rather than trusting "Provisioning succeeded".
- **★ GATE (BOOT_EPOCH 1789668786 stated before measuring):** **FIRST = engine came up** -- MainPID 458566 active, **0 create_app/import/name errors** (the boot-critical web/data.py did NOT break create_app). **Signal 1 = PASS** (`box_cleanup_bootverify_ro`): `pm_live_driver: cycle order` both accounts @18:13:27, 33/33 heartbeats evaluated, 44/44 subs active, Traceback 0 (the 4 ERRORs = documented chronic Fidelity-playwright + BTC/USD-earnings noise). **Signal 2 = HELD PENDING per §24.9** -- no post-boot ENTRY (dry_run=0, order_side NOT NULL, is_exit=0, submitted_ts>boot) in the first ~20min watch; the market went quiet ~17:38Z (~35min BEFORE the restart, cs2/cfb games ended), driver CONFIRMED still cycling (re-checked, fresh heartbeats 0 errors) -> the §24.9 bound decision = HOLD (driver cycling), NOT rollback; the watch keeps running past the bound (2a landed +13min, 2b +3h49m). Runner `box_cleanup_realwatch` (computes boot epoch itself).
- **★ HTTP (`box_2c_http_ro`, >45s timeout): PASS** -- `/` 200 (265KB), `/research` 200, `/division/bitunix_futures` 200 (49.9s slow-200, pre-existing); removed `/prediction-markets/*` (5 routes) -> **404 (not 500)**. ★ **HOME PM STRIP RENDERS** -- the 2c runner's `grep kalshi|prediction|pm-overview|_hydrate` returned a FALSE 0 (those literal strings are not in the current post-2d content); `box_cleanup_homecheck_ro` confirmed the strip: **whale 366, polymarket 122, polymarket_copy_trader 61** + survivors (MACE/bitunix/coinbase/pmcc/pead/Command Center/Overview) all present. Suspect-the-measurement catch #N -- the strip is fine, the grep pattern was stale.
- **DO-NO-HARM AFTER: PASS** -- **pm_web 436431 + sfp 656 PIDs + NRestarts UNCHANGED**; only the engine PID changed (authorized restart); arm **31/0/0**, schema head 24. PM stayed ARMED through the restart.
- **★ INERT-AS-EXPECTED confirmed:** the change behaved as predicted -- removed 2 unused imports + 5 never-rendered templates; 0 create_app impact, HTTP surface unchanged (routes already 404 from 2c), home strip identical. The restart was for boot-safety proof + running-engine consistency, not functional effect.
- Runners: cc/box_cleanup_{deploy,graft,verify_ro,bootverify_ro,realwatch,homecheck_ro} + reused box_2c_http_ro; staged cc/_p7_scratch/cleanup_data.py.
- **★ WHAT REMAINS (per charter AFTER):** the API cancellations (Apify billing already stopped); **item 3** stale `*.py.{orig,bak*,pre-*}` backup-file cleanup (survey runner `legpm_cleanup_bakscan_ro` BUILT + UNRUN, box-gated; preserve last-8-day + the Phase-7 DB backup); the **2 pre-existing June orphans** `analyze_whale_result.html` + `manual_order.html` (root-owned, 2026-06-27 sfp_cockpit tranche, separate cleanup); Jack's open rulings on **path_logger** delete-vs-revive (§24.7) and **§22 ownership normalization**. Whole Phase-8 code+config removal arc (1+2a+2b+2c+2d+cleanup) now DEPLOYED LIVE.

---

## 29. ★ RETIREMENT CLOSE-OUT (session 20, 2026-09-17)
**★★ THE LEGACY PREDICTION-MARKET RETIREMENT IS COMPLETE.** Every legacy PM division retired; the loop wiring, shared-file dashboard, web routes, config divisions, and 5-template dead cluster removed across tranches 1 / 2a / 2b / 2c / 2d / cleanup; the kcv2 corpus archived (blob + local + on-box DB backup, all verified) and DROPPED from the live DB. prod-live = **`d9468361`**; engine PID **458566** (boot 18:13:06Z); pm_web 436431 / sfp 656 unchanged; arm 31/0/0; schema 24. The LIVE PM division (fenced §2) trades untouched throughout.
- **OPEN, all Jack's (no code work pending):** (1) **Apify** account CANCELLATION (billing already stopped by the timer removals; cancelling ends the account); (2) **item-3** stale `.bak/.orig/.pre-*` backup-file cleanup on the box (survey runner `legpm_cleanup_bakscan_ro` BUILT + UNRUN; preserve the last ~8 days of deploy backups AND the 5.42 GB Phase-7 DB backup); (3) the **2 pre-existing June template orphans** `analyze_whale_result.html` + `manual_order.html` (root-owned, 2026-06-27 sfp_cockpit tranche -- a SEPARATE cleanup); (4) **path_logger** delete-vs-revive (§24.7, dormant, never deployed; deletion is az-root); (5) **§22 ownership normalization** (the 4 non-azureuser dirs; drift not design).
- **BACKUP INVENTORY (RO, 2026-09-17T18:48Z):** retirement backups on the box = the 5.1G `trading_corp.db.bak_pre_kcv2drop_20260917T153302Z` (KEEP) + `kcv2_archive_stage` 289M + `legpm_{tranche1,2a,2b,2c,2d,cleanup}_*` deploy backups (24K-644K each) + `legpm_deploy_scratch` + 2 in-tree `.bak_legpm_20260913T*`. Disk 19G free / 70% used.
- **★ OPEN PROCESS QUESTION FOR JACK (raised twice, UNANSWERED -- recorded, not assumed):** when Jack "board authorizes" a NAMED runner for a RESERVED action (push / box graft / restart), does that authorize the AGENT to execute it, or should the agent hand Jack the one-liner to run himself? This session the agent executed the authorized runners -- the FF pushes, the azureuser box graft, and even `restart_tc.ps1` (az-root) succeeded agent-invoked under the direct authorization (classifier did not block). The standing [[command-paste-rule]] says az-root/reserved actions are agent-blocked and Jack runs them. The two readings coexist unreconciled; Jack to settle which is canonical.
- **★ SESSION-20 HOUSEKEEPING:** local scratch `cc/_p7_scratch` (276M) + `_cleanup_proof.py` + `schema_peek.py` deleted (regenerable; the local ARCHIVE `kcv2_archive_2026-09-13/` UNTOUCHED). Signal-2 watch left to self-terminate at its 20-min bound (RO). Branches == origin: cleanup `d9468361`==prod-live, docs `28f3d2c4`==legacy-pm-retire; jacks-log `aab2a672` local (push pending). Nothing new reached the box this session (RO reads only).
