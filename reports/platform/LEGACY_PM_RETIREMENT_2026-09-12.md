...
### DEPLOYED / BUILT
- **Tranche 2a (§21; §24.8) -- DEPLOYED LIVE 2026-09-16 (first of two windows):** prod-live `ed6d9b83 -> ae1fef05` (FF) + box graft + engine restart; Signal 1 PASS, Signal 2 report-and-hold. Full result §24.8.
...
- **★ POST-RESTART GATE:** **Signal 1 = PASS** -- PM live driver wired + cycling within ~1 min and continuously for 21+ min (task heartbeats fresh both accounts, 33 category heartbeats `state=evaluated`, 0 errors, live `polymarket-data-api positions` journal fetches). **Signal 2 = did NOT fire** -- no `dry_run=0` order in the 20-min bound (21:09:28Z); last order 20:40:57Z was pre-boot. **Per the split-authority rule this is report-and-hold, NOT auto-rollback** (driver confirmed cycling; the window was a normal quiet placement gap -- cadence had thinned to 2/hr; Ph6 recorded >20-min gaps in active play). Agent recommendation: HOLD (graft verified clean); operator ruling pending.
...
- ★ **2b CLEARANCE:** `origin/prod-live == ae1fef05` is now TRUE (2a landed), so 2b's parent is present and `git push origin legacy-pm-tranche2b-2026-09-16:prod-live` would be a clean FF. **BUT 2b's second window is GATED on Jack accepting 2a's verification** (Signal 1 PASS; Signal 2 report-and-hold). Until Jack rules 2a verified + opens the second window, 2b stays local/unpushed. Do NOT push 2b on the strength of Signal-1 alone without Jack's go.
