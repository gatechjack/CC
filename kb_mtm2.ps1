...
$cmd = @'
echo KQ_START
V=$(az keyvault list --query '[0].name' -o tsv </dev/null 2>/dev/null)
if [ -z "$V" ]; then U=$(systemctl cat trading-corp.service 2>/dev/null | grep -oiE 'KEY_VAULT_URI=[^ ]+' | head -1 | cut -d= -f2-); V=$(echo "$U" | sed -E 's#https?://##; s#\..*##'); fi
echo VAULT_RESOLVED=$V
export KEY_VAULT_URI=https://$V.vault.azure.net
echo KVURI_SET=$KEY_VAULT_URI
cd /home/azureuser/trading_corp && ./venv/bin/python - <<'PYEOF'
...
for k,d in sorted(pos.items(), key=lambda z:z[1]['first_ts']):
    ...
    p=d['p']
    print(('BOOKED' if is_booked else 'PENDING'),'|',tk,'|side=',sd,'|entry_lp=',p.get('limit_price'),'|qty=',p.get('qty'),'|lp_range=',((min(d['lps']),max(d['lps'])) if d['lps'] else None),'|n_emis=',d['n'],'|first=',d['first_ts'][:16],'|last=',d['last_ts'][:16],'|leg=',p.get('leg'),'|type=',p.get('kalshi_arb_type'),'|set=',p.get('kalshi_arb_set_id'),'|leg_date=',p.get('leg_date'))
...
async def live():
    ...
    try:
        ...
        print('load_secrets_err:',repr(e)); s=None
    kid=(getattr(s,'kalshi_karen_api_key_id',None) if s else None) or os.environ.get('KALSHI_KAREN_API_KEY_ID')
    pem=(getattr(s,'kalshi_karen_private_key_pem',None) if s else None) or os.environ.get('KALSHI_KAREN_PRIVATE_KEY_PEM')
    if pem and '-----BEGIN' in pem and '\n' not in pem: pem=pem.replace('\\n','\n')
