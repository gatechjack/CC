"""Tests for the tasty_options_iron_condor strategy module (step 9).
...
async def test_scan_skips_when_vix_above_30(agent):
    with patch(
        "trading_corp.agents.strategies.tasty_options_iron_condor.get_vix",
        return_value=42.0,
    ):
...
async def test_scan_returns_empty_when_iv_rank_below_threshold(agent):
    with patch(
        ...
        "trading_corp.agents.strategies.tasty_options_iron_condor.calc_iv_rank",
...
async def test_scan_constructs_ic_on_happy_path(agent):
    ...
    with patch(
        ...
        "trading_corp.agents.strategies.tasty_options_iron_condor.calc_atm_iv",
...
def _seed_open_ic(
    agent: TastyOptionsIronCondorAgent, *,
    combo_id: str = "ic-1",
    symbol: str = "SPY",
...
# ---------------------------------------------------------------------------
# NEW: Empty-watchlist guard
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_scan_with_empty_watchlist_returns_empty_and_logs(
        strategies_yaml, macro_cal, exdiv_cal, tmp_db, caplog):
    """Empty watchlist → scan early-returns; no provider/broker calls."""
    _ensure_schema(tmp_db)
    # Rewrite config with watchlist: []
    strategies_yaml.write_text(
        strategies_yaml.read_text().replace("watchlist: [SPY]", "watchlist: []"),
        encoding="utf-8",
    )
    agent = TastyOptionsIronCondorAgent(
        strategies_yaml=strategies_yaml,
    ...
    with caplog.at_level("INFO"):
        combos = await agent.scan(broker)
    assert combos == []
    assert any("watchlist is empty" in r.message for r in caplog.records)


# ---------------------------------------------------------------------------
# NEW: strict_universe class attribute
# ---------------------------------------------------------------------------


def test_strict_universe_is_false():
    """Tasty Options watchlist is permissive — confirms grader gate-1 knob."""
    from trading_corp.agents.strategies.tasty_options_iron_condor import (
        TastyOptionsIronCondorAgent,
    )
    assert TastyOptionsIronCondorAgent.strict_universe is False
