...
# --------------------------------------------------------------------------
# template render — the three card states + un-buildable
# --------------------------------------------------------------------------
...
def _render(**ctx) -> str:
    ...
    real = (tdir / "approval_pmcc_combo_detail.html").read_text(encoding="utf-8")
    env = Environment(loader=DictLoader({
        "approval_pmcc_combo_detail.html": real,
        # minimal stub base so only the content block renders
        "base.html": "{% block title %}{% endblock %}{% block content %}{% endblock %}",
    }))
    env.filters["strike"] = lambda v: ("%.2f" % v) if v is not None else "—"
    env.filters["et_hms"] = lambda v: str(v)
    view = {
        "combo_id": "abc12345", "short_id": "abc12345", "symbol": "AAPL",
        "strategy": "robinhood_pmcc", "division": "robinhood_pmcc",
        "intent": "roll_short", "intent_label": "Roll", "direction": "credit",
        "net_price": 0.58, "contracts": 1,
        "leg_groups": [{"label": "Legs", "legs": []}],
        "added_at": "t", "leg_count": 2,
    }
    base = {"view": view, "combo_is_live": False, "earnings": None,
            "estimate": None, "estimate_reason": None}
    base.update(ctx)
    return env.get_template("approval_pmcc_combo_detail.html").render(**base)


def test_render_blocked_no_approve_button_shows_date_and_rec():
    html = _render(earnings={
        "kind": "blocked", "offer_roll": False, "verified": True,
        "date": "2026-08-05", "flag": None, "caveat": None,
        "recommendation": ("Earnings 2026-08-05 — let the current short call expire, "
                           "then sell a new call after earnings is announced and the "
                           "stock has moved."),
    })
    assert "Approve combo" not in html            # roll not offered
    assert "Approve disabled" in html
    assert "2026-08-05" in html                   # verified date shown
    assert "let the current short call expire" in html
    assert "Reject" in html                       # reject still available


def test_render_unverified_has_approve_and_flag():
    html = _render(earnings={
        "kind": "unverified", "offer_roll": True, "verified": False, "date": None,
        "flag": "earnings date unverified — confirm before rolling",
        "recommendation": None, "caveat": None,
    })
    assert "Approve combo" in html
    assert "unverified" in html and "confirm before rolling" in html


def test_render_clear_shows_estimate_with_strike_and_expiry():
    html = _render(
        earnings={"kind": "clear", "offer_roll": True, "verified": False,
                  "date": None, "flag": None, "recommendation": None, "caveat": None},
...
def test_render_unbuildable_shows_reason_no_estimate():
    html = _render(
        earnings={"kind": "clear", "offer_roll": True, "verified": False,
                  "date": None, "flag": None, "recommendation": None, "caveat": None},
        estimate_reason="Live estimate unavailable — quotes not returned; no order sent.",
    )
    assert "Live estimate unavailable" in html
    assert "Roll estimate" not in html            # no estimate block rendered
