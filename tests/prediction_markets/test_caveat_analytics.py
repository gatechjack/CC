...
def test_migration_004_schema(tmp_path):
    ...
    assert maxv == 6                                        # head of the migration chain (001-006 incl. CP3a 005/006)
...
def test_migration_004_idempotent_on_p1_shaped_db(tmp_path, monkeypatch):
    ...
    monkeypatch.undo()                                          # restore full MIGRATIONS (adds 004+005+006)
    db.init_db(p)                                               # apply 004+005+006 on the v3 DB with data
    ...
    assert v_before == 3 and v_after == 6 and cnt == 6
