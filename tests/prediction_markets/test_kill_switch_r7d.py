...
async def test_r7d_disarm_blocks_exit_through_the_driver(tmp_path):
    ...
    with db.connect(p) as conn:
        # seed a FILLED holding on the exit signal's wallet (per-wallet holding guard); persists for the cycles below
        conn.execute("INSERT INTO pm_subdivision_order (account_id,category,wallet,ticker,outcome_leg,is_exit,"
                     ...
                     (ACCT, CAT, exit_sig.wallet, T_TOR, "yes", NOW, NOW)); conn.commit()
