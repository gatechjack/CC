...
## PEAD earnings-watcher timeout — ✅ FIXED 2026-07-31 (deferred deeper options only)

`pead-earnings-watcher.service` (ISOLATED side-process; display cache for `/telemetry/pead`; engine unaffected) timed out on every run 2026-07-29→31. **Root cause = SEASONAL:** peak Q2 earnings season doubled the in-universe reporter count (676→1,248) past what the serial ~1.2s/name loop fits in the 900s ceiling. **FIXED + VERIFIED LIVE 2026-07-31** (full record: `runbooks/deploy_log.md` 2026-07-31 entry): (1) `TimeoutStartSec` 900→2400 (40min, immediate unfreeze), (2) slot-split — the 11:00 UTC run does shard 0, the 21:00 run shard 1 (stable `crc32(code)%2`; AM∪PM=full set, each name refreshes once/day). Marker/upsert-at-end + mode=ro invariants intact. **The peak is seasonal and recedes** as Q2 earnings pass; both changes are safe year-round. **DEFERRED (only pursue if growth outpaces the 40-min budget even sharded):** parallelize the per-name fetches (thread pool ~8-12; the real ~10× headroom fix), cross-run fundamentals cache (EPS/facts change slowly), or drop yfinance→EODHD-EOD for price/volume (cuts a round-trip + retires a yfinance-as-primary debt). **Priority: P4 / low (fixed; deeper work optional, operator-queued).**
...
## robinhood_pead per-order notional cap is a NO-OP (real-money) — OPEN (filed 2026-07-22, operator-attention, DO NOT auto-fix)

Surfaced read-only during the PMCC live-execution investigation. `robinhood_pead` (LIVE real-money since 2026-06-24, account 680725082) has **no enforced per-order dollar cap** beyond `position_pct × equity` (0.10): (1) the RiskAgent per-trade notional cap is a no-op — PEAD market orders have no `limit_price` → `ref_price=0.0` → `risk.py:213-218` skips the cap; `risk.yaml` also sets `per_trade_risk_pct: 1.0` (off) + `max_drawdown_disabled: true` for pead (deliberate — a DD auto-flatten would force-exit the drift book). (2) `auto_max_notional` / `auto_execute_caps` are unenforced because PEAD **bypasses ceo_graph** (calls `data_exec.place()` directly, unlike PMCC/IC). The intended per-trade control is the `pead_pressures` exit-engine stops. **Recorded for operator attention — do NOT propose or apply a fix unprompted; operator decides whether/when. Out of scope for the PMCC live-execution workstream.** Memory [[pead-notional-cap-noop-2026-07-22]]. **Priority: operator-attention / real-money.**
...
## RH-auth dashboard (web layer) still un-reconciled to git — OPEN (filed 2026-07-22, LOW-pri git-hygiene)

The 2026-07-18 RH-auth 401 self-heal was deployed prod-direct across 5+ files. The ENGINE layer is now reconciled to git byte-exact (`main.py`/`secrets.py`/`robinhood.py`/`data_exec.py`; commits `f1b8a86`/`e2e2fad`/`557a39f`). **Still un-reconciled: the WEB layer** — `trading_corp/web/routes.py` (the `/api/rh/session-health` route + `Refresh RH pickle` button handler) + templates `home.html` + `rh_session_panel.html`. **Non-boot-critical:** a fresh git deploy boots and self-heals; only the dashboard RH panel/button are absent. Reconcile via the same transcription discipline (capture prod off-box → branch → byte-exact → LF-norm md5 == prod → suite → commit; NO deploy — prod already has it). **Before touching any one of these, enumerate the full patch set (`deploy_rh_auth/*.patch`) — see the partial-reconcile guardrail below.** Memory [[rh-auth-git-reconcile-2026-07-22]]. **Priority: P3 / low (git hygiene, not runtime).**
