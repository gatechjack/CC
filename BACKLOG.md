...
12. **C-1 APIFY_API_TOKEN rotation** — Apify console → KV update. Operator-portal step. **⚠️ ELEVATED to P1 — 2026-05-29:** the token VALUE (40-char hex) was surfaced in a test-failure transcript during the C-1 bitunix session (a `assert x in _REDACT_LITERALS` repr'd the loaded-secret set; see memory `[[no-membership-assert-on-secret-collections]]`). Apify tokens do **not** self-rotate, so the exposure is **live until rotated**. Do next session, right after bitunix lands.
