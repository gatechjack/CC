...
## P3 — Throwaway-script preservation: extract working PS scripts from 2026-05-31 tastytrade rotation into `scripts/c1_tastytrade_rotation_helpers.ps1` (filed 2026-05-31)

Surfaced by C-1 item 13 rotation 2026-05-31 ~18:31 UTC. The session derived 7 working PowerShell forms across Step 1 sub-steps 4-6 + Step 2 sub-steps 2-4 + Check 4, after working through 4 PowerShell paste/quoting failure modes (see `[[tastytrade-credential-rotation-2026-05-31]]` "PowerShell quirks encountered"). The runbook (`runbooks/tastytrade_oauth_rotation.md`) intentionally documents procedure not literal scripts, so future operators would re-derive these — repeating the failure modes.

**Scope:** Extract the empirically-working forms into a reusable helper script. Suggested location: `scripts/c1_tastytrade_rotation_helpers.ps1`. Contents:

1. **JWT decode helper** (`Get-TastytradeJwtClaims [-RefreshToken <string>]`) — printout: `scope`, `exp`, `iat`, `iss`, `sub`, other-claims. Uses base64url + PSReadLine SaveNothing guard.
2. **OAuth authorization-code grant exchange** (`Invoke-TastytradeTokenExchange -ClientId <id> -ClientSecret <secret> -Code <code> -RedirectUri <uri>`) — calls `https://api.tastytrade.com/oauth/token` with **form-encoded** body (NOT JSON — this is the gotcha pinned in this rotation), reads full response stream on error (PS 5.1 WebException handling).
3. **Atomic env-file write to prod via `az vm run-command`** (`Set-TastytradeProdEnvFile -ClientSecret <secret> -RefreshToken <token>`) — uses single-quoted PS string + `$(...)` preservation pattern + single-line bash with `;` separators (not multi-line; PS 5.1 native-arg-passing splits on newlines). Document the Azure activity-log capture as accepted leak surface.
4. **Registry write with SecureString-decode pattern** (`Set-TastytradeRegistryEnv`) — `Read-Host -AsSecureString` + BSTR-decode-then-zero, with explicit `$env:` sync (gotcha 2 fix).
5. **In-process == registry parity check** (`Test-TastytradeRegistryParity`) — Check 4 from runbook.
6. **Live-grant decode probe against prod** (value-safe — outputs scope/exp/iat only, never the value).
7. **Globals scrub helper** (`Clear-TastytradeGlobals`).

**Why P3 (not higher):**
- The runbook (`runbooks/tastytrade_oauth_rotation.md`) is canonical and complete; this is helper-script convenience, not a procedure gap.
- Next tastytrade rotation isn't time-pressured — operator's standing credential-hygiene cadence will set it.
- The forensics in `[[tastytrade-credential-rotation-2026-05-31]]` already document the 4 PowerShell quirks; an operator reading that memory before the next rotation can avoid the same dead-ends without the helper script.

**Why P3 (not skipped):**
- The 2026-05-31 rotation spent disproportionate time on PowerShell paste mechanics (here-string termination, `$(...)` interpretation, multi-line `--scripts` arg splitting). Future-operator's-time-cost is real if these aren't pinned.
- The helper script can be tested against `is_test=True` (TT CERT endpoint) without affecting prod credentials — easy to validate.

**Out of scope:** Anything that changes the runbook procedure. Helper scripts are runbook-aligned, not runbook-replacing. Runbook edits remain Board-approval per CLAUDE.md §4.

**Pickup pointer for whoever picks this up:** start from `[[tastytrade-credential-rotation-2026-05-31]]` "PowerShell quirks encountered" section; the 7 working forms can be reconstructed from the 2026-05-31 session chat transcript and from this file's deploy_log entry citations.
...
**Headline:** Operator asked whether kalshi_weather's 100%-NO book is rational (YES considered, no edge surfaces) or structural (YES excluded/asymmetrically gated). Answer: **BOTH, and the bigger finding is the NO book itself is negative-EV.** YES *is* reached (3,316 natural-YES candidates, 25% of 13,197 post-cutoff evals) but **0 ever place** vs 176 placed NOs — pruned by a stack of asymmetric gates. Meanwhile the NO bets that DO place run at **59.2% WR on 76 settled (avg entry $0.712 → break-even needs 71.2%) = −$0.12/contract, −$9.14 gross (paper, no fees in weather path).** The strategy's placed book is structurally losing; the YES-suppression is a second-order issue on top of that.

**What we measured (all post-cutoff window `ts >= 2026-05-26T01:08:00`):**

1. **TRACK A — bias-offset post-deploy sanity: PASS.** All 9 spring `fully_validated` cells (KAUS/KDEN/KDFW/KHOU/KMSP/KMSY/KNYC/KOKC/KSAT) firing with correct offsets, all `fully_validated`, all non-zero. KDEN = −3.187°F exact match. 438–719 rows/cell over ~47h; first-fire rollout completed within 3h17m of restart. No anomalies.

2. **YES never places — funnel (3,316 natural-YES candidates):** 79.6% die on `edge < 10% min_divergence_pct`; 11.4% on `entry_below_floor: yes (≤$0.10)`; 8.6% on bucket-guard `block_yes_forecast_outside`; 0.4% on Kelly min. → **0 fired as YES.** Three structural asymmetries identified (all in code, all documented/intentional): (a) bucket-guard one-way valve (`_weather_math.py:326-390`) — blocks YES-against-forecast as a "σ-smearing artifact", never the reverse; (b) entry-price floor 5× asymmetric (`min_yes_entry 0.10` inclusive vs `min_no_entry 0.50` strict, `_weather_math.py:393-425`); (c) all 9 spring bias-offsets are NEGATIVE, pushing "greater"-direction forecasts into the NO zone.

3. **Bucket-guard hypothetical PnL (is the guard catching losers or suppressing winners?):** `block_yes_forecast_outside` 287 rows/57 tickers, only **4 settled, 0/4 would-win** → directional support for "guard catching real losers" but n far too small to declare; all 4 were $0.01-YES sigma-smearing tails. `block_no_yes_too_expensive` 43/13, **0 settled** — can't evaluate yet. Revisit when 2026-05-27/28 markets settle (n≥30 expected on block_yes within ~1 week).

4. **`flipped_no_to_yes` rescue path is DEAD BY CONSTRUCTION** (provable + empirically 457/457). The guard flips NO→YES, then Kelly sizes the flipped YES against `prob_outcome = prob_yes` at `share_price = yes_ask`; since `implied_yes == yes_ask` (`kalshi_weather_arb.py:753`) and the natural path chose NO precisely because `prob_yes < implied_yes`, the full-Kelly numerator reduces to `(prob_yes − yes_ask) < 0` → `max(0,·)=0` → below `min_usd` → skip. Empirical: 457 flagged rows, **kelly_full_pct=0.0 on every single one, 0 fired, 0 exceptions.** The flip ceiling (0.70) bounds absolute YES price but never overrides the relative check. The audit LABEL fires; no contract can ever be bought.

5. **The "Chicago model-vs-market disagreement" is NOT a model error.** KXHIGHCHI-26MAY26-B83.5 (implied 0.99 / prob_yes 0.118) and -26MAY27-B81.5 (implied 0.99 / prob_yes 0.156): the gap is **horizon collapse + bucket/σ ratio**, not pathology. At 2.6h / 0.41h before close the market has collapsed onto the observed daily high while our model still applies forecast σ. A 1°F "between" bucket against σ=2.5–3.33°F mechanically caps prob_yes at ~12-16% even with a perfect center-of-bucket forecast — math reproduces to 3 decimals via the Gaussian CDF. KMDW spring `bias_offset=0` (no spring entry in `BIAS_OFFSETS_V1` — the −1.370 is summer, did not fire), `coord_source=yaml_verified`, station_id correct. Forecast was directionally accurate (83°F vs actual ~83-84°F; adjacent B85.5/B87.5 both settled NO and we WON both).

**Items NEWLY OPEN (filed from this session):**

1. **kalshi_weather NO-book negative-EV (P1/P2 — the real signal).** 76 settled post-cutoff NOs: 59.2% WR, avg entry $0.712, break-even 71.2%, −$0.12/contract, −$9.14 gross (paper). Directional accuracy is real (>50%) but insufficient vs the high NO entry price. This is bigger than the YES question and bigger than any single guard. Decision needed: is the edge real but mis-priced-entry, or is the whole forecast-vs-Kalshi-weather thesis marginal? Needs a larger settled sample (this is the first real post-bias-offset cohort).
2. **`flipped_no_to_yes` is dead-code instrumentation (P3).** The guard logs ~457 "rescues"/window that can never place. Either fix the sizing premise (the flip should size against the corrected belief, not re-run the rejected check) or remove the misleading label so the audit stops implying rescues happen. NOT causing mis-placed orders — pure observability noise + a latent design contradiction.
3. **Near-zero-horizon between-bucket evals produce phantom edge (P3).** Below ~3h horizon, between-direction markets collapse onto observation; we log 83-87% "edge" that's structurally un-actionable, burning eval cycles + noising the audit. Candidate: a horizon floor for between markets (e.g., skip eval below ~4h) — needs design discussion, NOT a quick fix.
4. **YES structurally pruned (characterized; design, not bug).** 3,316 candidates → 0 placed. Documented above. Open question is whether the design is RIGHT given finding #1 — revisit jointly with the negative-EV decision, not in isolation.

**Verification caveat:** settled-sample sizes are tiny (4 settled on block_yes, 0 on block_no, 76 on the NO baseline). All directional only. The right re-look is ~1 week out when 2026-05-27/28 weather markets settle.

**Memory updates this session:** UPDATED `project_kalshi_weather_bias_offset_v1_live.md` (the early 3/3-WINS sample is superseded by the 76-settled / 59.2% / negative-EV cohort). NEW `project_kalshi_weather_yesno_structure.md` (the findings above). NEW `feedback_prediction_market_horizon_collapse.md` (near-settlement large divergence = market-on-observation vs model-on-forecast, not actionable edge — generalizes across weather/crypto settling markets).
...
## P3 — Fidelity startup login flakiness on `trading-corp` restart (ANOMALY, RECURRING)

Recurring side-effect of every `trading-corp` restart: Fidelity's headless login fails with `'Sorry, we can't complete this action right now'` (bot detection / stale CSRF / rate limit) → `broker_fallback_to_paper` for `fidelity_joint` and `fidelity_401k`. Observed again on the 2026-05-29 19:05:59 UTC C-1 bitunix rotation restart (independent of the rotation — same pattern logged in `deploy_log` 2026-05-29 ~16:55 UTC and the 2026-05-27 EOS snapshot). Fidelity is read-only paper today, so impact = dashboard $0 for those divisions until a future restart re-establishes the session. Low-priority while Fidelity stays read-only paper; revisit when a write-path is added. Don't bundle the fix-space (retry / cache-warming / headed login) into other restart sessions — it's a focused side-quest, and currently a future restart "usually works".
...
7. ~~**C-1 BITUNIX_FUTURES_API_KEY / SECRET rotation**~~ **✅ RETIRED 2026-05-29 ~19:06 UTC** — rotated end-to-end. KV versions: KEY `9b33309a64da4855bb10128243c6b499`, SECRET `51b5020363184d5491fa77619bec5ffa` (vault `kv-tc-vtwbowt3wtkpy`). Bitunix portal = **REPLACE-ON-CREATE** (one-key-slot; old auto-invalidated; no explicit revoke needed). Old-key dead empirically (10003 from old process pre-restart + FAKE-creds synthetic probe matched). Trade-only / withdraw-disabled / IP-whitelisted to `20.51.145.253`. See `deploy_log` 2026-05-29 ~19:06 UTC + `[[bitunix-credential-rotation-2026-05-29]]`. C-1 progress now **4/13+** (webhook ×2 on 2026-05-27 + bitunix ×2 on 2026-05-29).
...
12. ~~**C-1 APIFY_API_TOKEN rotation**~~ **✅ RETIRED 2026-05-29 ~20:08 UTC** — rotated end-to-end after the token VALUE was exposed earlier the same day in the C-1 bitunix test-failure transcript repr (see `[[no-membership-assert-on-secret-collections]]` — standing rule designed out the assert shape; was P1-elevated on `c1-bitunix-cred-rotation` branch's `a868f46` annotation). KV new version `ebf41050d1d14a65982b4ce0e00b40d7` (vault `kv-tc-vtwbowt3wtkpy`). Apify portal = **REVOKE-REQUIRED** (multi-token; flat personal tokens only — no scope UI in this account/tier). Old "Kalshi-" labeled token revoked on apify.com after new-token auth verified (value-blind probe: `http=200 username=GT_Jack`); synthetic FAKE-token probe matched the rejection envelope (`http=401 user-or-token-not-found`). **Zero code change** — all apify redaction + KV surfaces were already shipped. See `deploy_log` 2026-05-29 ~20:08 UTC + `[[apify-credential-rotation-2026-05-29]]`. C-1 progress now **5/13+**.
13. ~~**C-1 TASTYTRADE_PROVIDER_SECRET + TASTYTRADE_REFRESH_TOKEN rotation**~~ **✅ RETIRED 2026-05-31 ~18:31 UTC** — rotated end-to-end against the P1 2026-06-12 ceiling. New Client Secret regenerated in the TT developer portal (REVOKE-ON-CREATE — old secret died at portal-rotate-time, orphaning the 5 prior grants visible in "Manage OAuth Grants"). New matched refresh token minted via authorization_code grant against `https://api.tastytrade.com/oauth/token` (form-encoded body required, not JSON — the SDK's refresh_token grant uses JSON but the bootstrap grant differs; pinned in deploy_log). JWT scope = `read trade` (matches 5/24/2026 prod grant; no silent downgrade — dry-run order returned `422 margin_check_failed` not `403 insufficient scopes`). Prod env file `/etc/trading-corp/tastytrade.env` rewritten via `printf | sudo install`; mtime advanced `2026-05-22 16:25:14 → 2026-05-31 18:25:50` UTC. Service restarted (MainPID `1918098 → 1961212`); SDK `Session.refresh()` succeeded on startup (`DEBUG tastytrade: Refreshed token, expires in 900s` at 18:31:12 UTC). Local Windows registry rotated via `[Environment]::SetEnvironmentVariable("User")` + in-process sync (registry value pre-rotation was corrupt junk — PS-expression fragment from a prior session, no JWT shape; local-dev verification scripts had been silently broken; not a prod issue). Tasty smoke `scripts/tasty_sandbox_smoke.py` 4/4 PASS post-rotation; IC division implicit-passed via the smoke's `GET /option-chains/SPY` probe. **Zero code change** — all redaction + env-loader surfaces were already shipped on `main`. **No secret values entered the Claude Code session** — operator handled values exclusively in own SaveNothing PowerShell with Read-Host SecureString + BSTR-decode-then-zero pattern. See `deploy_log` 2026-05-31 ~18:31 UTC + `[[tastytrade-credential-rotation-2026-05-31]]`. C-1 progress now **6/13+**. See `[[tastytrade-refresh-token-no-self-rotation]]` for the underlying SDK finding (still load-bearing).
...
2. ~~**Push the trade-scoped TASTYTRADE_REFRESH_TOKEN to prod's `/etc/trading-corp/tastytrade.env`**~~ **✅ RETIRED 2026-05-31 ~18:31 UTC as a side-effect of C-1 item 13.** The C-1 rotation minted a fresh `read trade`-scope refresh token and atomically wrote it to both `/etc/trading-corp/tastytrade.env` (prod) and `HKCU:\Environment` (local registry). Service restarted; `Session.refresh()` succeeded with the new pair (journal `DEBUG tastytrade: Refreshed token, expires in 900s` at 18:31:12 UTC). tasty_options Phase 2 readiness for token-side is unblocked (other gates — Backtester approval, `auto_execute=true` flip — remain). See `deploy_log` 2026-05-31 ~18:31 UTC.
