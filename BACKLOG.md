...
## `auto_execute: false` does NOT gate every PMCC placement path — the dashboard/scout buttons bypass ceo_graph — OPEN (filed 2026-07-23, HIGH — pre-arming blocker)

Standalone, high-consequence finding (surfaced in the 2026-07-23 pre-arming readiness assessment). **`auto_execute: false` gates only the SCHEDULED paths** (pre-open scan + 15:00-ET terminal pass → `_run_order` → `ceo_graph.approval_node`; `_check_auto_execute` returns False at `ceo_graph.py:132-133` → Board interrupt at `:411`). **The dashboard "Approve & Execute" (`execute_pair_orders`, `routes.py:995`) and Scout-execute (`execute_scout_open`, `routes.py:1215`) routes bypass `ceo_graph` entirely** — the human click IS the approval, and they call `data_exec.place()` **directly, per-leg** (`routes.py:1123`); `auto_execute` is never consulted. **Practical consequence: on arming (`robinhood_pmcc` into `--live-divisions`), the same dashboard button that produced the 2026-07-23 paper fill would place TWO real single-leg orders, NON-ATOMICALLY, on the first click** — no `auto_execute`, no ceo_graph, no combo atomicity. (A `close_all` click likewise places a real `close_leap_urgent` 0.0 market LEAP sell, bypassing the `_LEAP_CLOSE_ACTIONS` forced-approval that only guards the scheduled path.) Today this is inert (division is paper — no order reaches a broker; see [[pmcc-paper-end-to-end-correction-2026-07-23]]), so it is a **hard pre-arming blocker, not a current exposure.** Fix shape (do NOT build): route the dashboard/scout execute through the atomic combo path + a real approval gate, OR explicitly accept the buttons as live-fire-on-click. **This is the linchpin item on the sequenced pre-arming list** in `planning/pmcc_live_execution_plan.md` (2026-07-23 amendment) — fixing it subsumes the per-leg partial-fill and naked-hung-limit variants. **Priority: HIGH / blocking-before-arming; operator-queued.**

## PMCC risk gate evaluates against a FABRICATED $100k balance — go-live defect, NOT a current real-money exposure — OPEN (filed 2026-07-22, severity corrected 2026-07-23, Phase-B candidate)

**★ SEVERITY CORRECTED 2026-07-23:** the original "real-money-adjacent" framing was WRONG. This is a valid **go-live** defect, **not a current real-money exposure** — no PMCC order reaches a broker (the division is paper: `robinhood_pmcc` is not in `--live-divisions`, every fill synthetic; see [[pmcc-paper-end-to-end-correction-2026-07-23]]). The fabricated-equity gate never gates a real order today. **It becomes real the moment the division is armed into `--live-divisions`** — at which point equity-relative caps would silently run against $100k instead of the real balance. Keep it as a hard go-live precondition, not a current-risk item.
...
## A division's LIVE/PAPER arming is invisible in config — decided only by the systemd ExecStart `--live-divisions` CLI arg — OPEN (filed 2026-07-23, MED — observability/safety)

A division's paper-vs-live **PLACEMENT** state is decided solely by the `--live-divisions` (+ `--brokers` + `--live`) tokens on the **systemd ExecStart line**, evaluated in `_build_broker_for_division` (`main.py:2460-2461`: `is_live_division = (mode==LIVE AND family∈--brokers) AND slug∈--live-divisions`). **`config/divisions.yaml` and `config/strategies.yaml` say NOTHING about paper/live** — verified 2026-07-23: `robinhood_pmcc` in divisions.yaml has `broker/enabled/account_filter` but **no `paper`/`execution_mode`/`standby`** key; strategies.yaml has `enabled/auto_execute` only. The arming is "documented" only in the argparse help (`main.py:150-156`), the E2·4 code comment (`main.py:2451-2462`), and the boot log line `Registered … broker for division=X (paper=Y)`. **Runtime surfaces are thin and do not cover this division:** `broker.paper` per division (in-process only); a *family-level* `paper` flag in the accounts view (`web/data.py:1208`); and a header badge (`web/app.py:271-312`) that shows `execution_mode` for **`bitunix_futures` only** (hardcoded — "Multi-division execution_mode visibility is intentionally out of scope"). **No status endpoint and no dashboard element reports a given division's arming** (routes.py has `/division/{slug}` pages + `/api/rh/session-health`, none report live/paper). CONSEQUENCE: an operator reading config cannot tell whether a division places live — the truth lives in one CLI string in the unit file — and the PMCC cockpit even renders a green "Executed · 2 placed" on a *paper* fill (see the sibling UI-rendering item). This is the misconception that produced the 2026-07-23 "TSLA executed but nothing reached Robinhood" report. **Fix shape (report-only, do NOT build):** (1) surface `execution_mode`/`is_live` per division from `data_exec.brokers[slug].paper` via a status endpoint (`/api/divisions/status`) and/or a dashboard tile; and/or (2) mirror the arming into config (a `live: true|false` per division in divisions.yaml) that `main.py` cross-checks against `--live-divisions` at boot and **warns on mismatch** — keeping ExecStart authoritative but making the state discoverable + diffable. Surfaced by the read-only PMCC-paper diagnosis. Memory [[pmcc-paper-end-to-end-correction-2026-07-23]]. **Priority: MED (observability/safety; operator-queued).**

## PMCC — liquidity gate rejects genuinely-liquid near-dated contracts; near-dated / black-sheep rolls silently unexecutable — OPEN (scoped 2026-07-23, HIGH)

Surfaced when the new deep-OTM rule made TSLA a 1-DTE roll_short candidate: the roll **aborts** (`pmcc_roll_aborted` reason=`sparse_chain_no_weekly`, missing_leg=new_short, `considered:126 liquid:0 target_date:2026-07-27`). **The contracts are NOT illiquid** — live TSLA 2026-07-27 calls (16:13 UTC, spot ~320.7): 0.25-delta targets **C335 (vol 7,942, OI 5, 2.85/2.90) / C340 (vol 4,848, OI 11, 1.83/1.87)** — 1–2¢ spreads, thousands traded. `_find_best_weekly` picks the EARLIEST roll-out expiry (`weekly_dates[0]`); rolling from a 1-DTE short lands on the freshest daily (07-27), the thinnest-OI expiry. 07-27 is a valid TSLA expiry (confirmed via chain); not a bad date, not the B7/60-DTE ceiling, not a data problem. **TWO SEPARATE DEFECTS in `_passes_liquidity` (`pmcc_robinhood.py:834`), scoped independently:**

**(a) `min_avg_options_volume: 10000` applied PER-CONTRACT — misuse, black-sheep-only. [fix candidate — ships with the message-fix]**
Read operationally in exactly ONE place: `:845` (per-contract branch). The key lives in `black_sheep.eligibility_criteria` (strategies.yaml:241–246) beside `min_options_iv` and `min_underlying_avg_volume` — all **underlying-eligibility screens** ("is this NAME suitable"); `min_avg_options_volume` = "does the name trade ≥10k options/day on average." **There is NO underlying-screen consumer** — the value was wired straight into the per-contract gate on 2026-05-01 (`b70b6a3`/`606254e4`), live ~2.7 months. **Historical proof of misuse:** in 2.7 months only TWO black-sheep roll-open legs EVER cleared the gate — MSTR delta 0.366 (07-02) + TSLA delta 0.607 (06-05), **both near-ATM halfway/breach rolls, both board_rejected**; **NO normal-cycle (~0.25-delta OTM) black-sheep roll has EVER passed** (OTM trades 3–8k, never 10k). (`pmcc_paper_run_readiness.py:138` fakes `volume=20000`/contract to pass — never tested real chain liquidity.) **Correct semantics:** a per-contract floor should be modest (non-BS `min_avg_volume=50`, or a few hundred); the 10k belongs — if kept — as a separate underlying screen (no code does this). **Blast radius:** the key feeds only `_passes_liquidity`→`_filter_liquid`→`_find_best_weekly` (:3796, rolls+opens) **and** `_find_best_leap` (:3688, LEAP) — PMCC + **black-sheep only** (TSLA/MSTR). A fix does NOT touch non-BS names or any other division.

**(b) `min_open_interest: 100` mis-calibrated for near-dated expiries — calibration, PMCC-wide. [needs an operator decision on approach]**
OI is a "real/established contract" proxy but is a STOCK that builds over a contract's life; volume+spread are the live signals. A fresh daily with 8k volume + 2¢ spreads carries OI 5–11 and is rejected. Options (unpicked): DTE-scaled OI floor / OR-against-volume / spread-based test / composite score. **Blast radius:** `min_open_interest` read only in `pmcc_robinhood.py` `_passes_liquidity`, used by `_find_best_weekly` + `_find_best_leap` — PMCC-only but **all names** (broader than (a)).

**Is any roll executable today?** Non-BS: only where the target strike has OI≥100 — the established Friday weeklies/monthlies, NOT the fresh near-dated dailies; effective floor ≈ "next established Friday weekly, DTE ≳ 7–14"; a 1-DTE roll fails. Black-sheep (TSLA/MSTR): the 10k gate blocks the normal 0.25-delta OTM target at EVERY DTE → **normal-cycle black-sheep rolls are effectively disabled entirely** (0 passes in 2.7 months). **Priority: HIGH** — the new deep-OTM rule now surfaces 1-DTE rolls, and they all dead-end here; separately, (a) means black-sheep normal rolls have never worked.

## PMCC — dashboard "Approve & Execute" bypasses the Phase A atomic combo path — OPEN (filed 2026-07-23)

`execute_pair_orders` (`web/routes.py:995`, the dashboard cockpit Approve button) dispatches **per-leg via `data_exec.place()`** in a loop (`:1057-1127`) — it never uses `pmcc_pending_combo_registry` / `place_combo`. **UI paths that can dispatch a PMCC roll, and atomicity:**
- **Dashboard cockpit "Approve & Execute"** → `/division/{slug}/pair/{symbol}/execute` → `execute_pair_orders` → **per-leg `place()`** — **NON-ATOMIC** (the path the operator used for TSLA).
- **Scan → PMCC combo card** → `/approvals/pmcc-combos/{id}/decide` → `dispatch_approved_ic_combo` → **`place_combo`** — **ATOMIC** (Phase A; only reachable when a roll_short is surfaced by the SCAN and rendered as a combo card).
- **Generic approval** → `/approvals/{order_id}/decide` → resolves a BoardDecision into the ceo_graph HITL → ceo_graph **per-leg `place()`** — **NON-ATOMIC** (handles advisory roll_leap legs; roll_short doesn't route here).

So a roll_short surfaced via the **dashboard** (e.g. today's TSLA, from `analyze_symbol` on card view) → Approve → non-atomic per-leg; Phase A's atomicity is reachable ONLY through the scan→combo-card flow. **Fail-closed guard DOES protect this path:** `place()` (`data_exec.py:131`) and `place_combo()` (`:684`) both call `_is_advisory_order` → a roll_leap approved from the dashboard button raises `AdvisoryOrderError` per leg (caught → execute_error), so it is refused, not placed. **Routing execute_pair_orders through the combo path would involve:** detecting the combo-tagged roll_short legs (2 legs sharing `pmcc_pair_id`) and routing them to `propose_pmcc_combo`/`place_combo` (pieces already exist) instead of the per-leg loop; single-leg actions (open/close_short) stay per-leg; roll_leap stays advisory. **Priority: P2 — scope known; decide before fixing.** Do NOT fix yet.

## PMCC — "no orders to place" message swallows the B4 abort reason — OPEN (filed 2026-07-23)

When `propose_orders_for_pair` returns `[]` because of a B4 abort, `execute_pair_orders` renders `_exec_no_action_html` (`web/routes.py:4683`): "Action `ROLL SHORT` does not require any orders. Position remains as-is." — **false** (the roll DID require orders; they were aborted for no liquid weekly). **The abort reason IS reachable from `execute_pair_orders`, not only in the audit write:** `_propose_roll_short` stores it in-memory on `self._last_weekly_diag` / `self._last_leap_diag` (`pmcc_robinhood.py:666`, set at `:3562`) AND writes an `_audit_roll_abort` event. `execute_pair_orders` holds `deps.pmcc_agent`, so after the empty return it can read `deps.pmcc_agent._last_weekly_diag` (reason, considered, liquid, target_date). **Fix shape (do NOT build):** when the action is a roll/close (not hold/watch) but `orders == []`, render the diag ("roll aborted: no liquid weekly — considered N, liquid 0, target 2026-07-27") instead of the "no orders needed" message — no new plumbing needed (the diag is already in memory). **Priority: P2 UX/observability** (couples with the config-observability + terminal-pass-heartbeat pattern below — deterministic inaction must be self-reporting). **Note:** the diag reason itself (`sparse_chain_no_weekly`) is the symptom of the liquidity-gate bug above.

## PMCC — deploy `pmcc_paper_run_readiness.py` to prod so the readiness check runs against what prod actually runs — OPEN (filed 2026-07-23)

`trading_corp/scripts/pmcc_paper_run_readiness.py` has never been on prod (the prod `trading_corp/scripts/` dir has the IC counterpart `ic_paper_run_readiness.py` but not the PMCC one). It's a standalone `__main__` report tool, not imported by the engine, so it's correctly excluded from engine deploys — but that means the readiness check has only ever run **against local config**, while prod carries drift (the copy-trader autopause blocks, and historically config that lagged git). Putting it on prod (a plain scp of the file into `trading_corp/scripts/`; no restart, no engine effect) would let the check verify **what is actually running**. Its own action, deliberately NOT bundled with the 2026-07-23 liquidity+black-sheep deploy. **Priority: P3 — operator-queued, standalone.**

## PMCC (and system-wide) — no way to read what config the RUNNING process loaded — OPEN (filed 2026-07-23)

Verifying a config-gated safety feature is actually live currently requires **re-running the loader against the on-disk file** (2026-07-23 deep-OTM early-roll deploy: constructed the `PMCCAgent` cfg path + ran the gate methods against the live `strategies.yaml` to prove `near_worthless_early_roll` reached `cfg` and the gate is not inert). That is sound inference — a deterministic loader on the unchanged booted file — but it is **inference, not observation**: the engine exposes no way to read what the running process actually holds in `self._cfg`. A YAML block present on disk but not reaching `cfg` would leave a gate **permanently inert** and look identical to "no eligible positions today."

Fix shape (do NOT build now): a **boot-time log line echoing the loaded PMCC config** (the resolved `robinhood_pmcc` sub-blocks — `zero_dte`, `near_worthless_early_roll`, `roll_cooldown` — as the process actually parsed them), OR a small read-only **config-dump endpoint** on the web server. Either makes config-gated deploys directly verifiable on the running process.

★ **This is the SECOND time a deploy needed evidence the system could not directly provide.** The first was B10's 15:00-ET terminal-DTE pass, solved by making it emit an **unconditional `terminal_dte_pass_done` audit row** (so a no-op pass is distinguishable from a pass that never ran). The pattern: a deterministic behavior that logs only when it ACTS is unverifiable when the correct outcome is **inaction** — add an unconditional heartbeat / echo. Apply the same lesson to config loading. **Priority: P3 observability.**
