...
## P2 — HTF gate support detection: verify mechanism + scope operator-curated S/R integration  *(NEW — 2026-06-01; surfaced by HTF proximity audit)*

**Surfacing:** the 2026-06-01 historical audit (`reports/2026-06-01_htf_proximity_hard_zero_historical_audit.md`, branch `htf-proximity-audit-2026-06-01`) found the proximity hard-zero rule is approximately NEUTRAL on N=157 over 2026-05-16 → 2026-05-31 (mean Δ+60m +0.016%, 73.5% block-held-loose). The aggregate-neutral headline may be conflated by the mechanical support-detection upstream of `bitunix_htf_regime.py:979-988` (`verdict.distance_to_support_pct`). Two small-N cohorts deviated sharply: `cvd_bull_flip` N=8 with 50% wrong-loose + mean Δ+60m +0.30%; BEAR/PREMIUM N=11 with 36.4% wrong-strong. Hypothesis worth testing: the model is identifying "support" at locations that have no corresponding operator-curated S/R zone.

**Operator framing:** the grey/pink boxes on the TradingView chart are the empirical "real support" reference — they reflect significant historical price-action interactions (multi-day structure, repeated tests, validated bounces/breaks). If the audit's neutrality is partly because the HTF gate's "support" doesn't coincide with these zones, fixing the detection source (rather than tuning the threshold) is the right lever.

**Investigation scope (this entry is for scoping, not implementation):**
1. **Verify how `distance_to_support_pct` is computed** upstream of `bitunix_htf_regime.py:979-988`. Pivots? Swing lows? MA? What lookback? Single-TF or composite?
2. **Quantify overlap** between model-detected support and operator-curated zones on the 155-event audit corpus. Sub-question: were the cvd_bull_flip rejections at operator-curated zones or at algorithmic-only "support"?
3. **Scope integration cost** for the operator-curated S/R zone source (TradingView indicator export, Pine script extraction, manual config file, external API). Cross-reference the ingestion sketches in the prior P3 entry (BACKLOG.md L5257) — that entry assumed manual entry / OCR for Pink Box image uploads as confluence input; this entry's question is whether the same source can REPLACE the gate's mechanical detection, not just augment it.
4. **Compare HTF rule calibration** with mechanical-only support vs operator-curated support on the same 155-event corpus. Is the per-cohort selectivity that's invisible at the aggregate (e.g., cvd_bull_flip) explainable by support-source mismatch?

**Out of scope:** implementation. If the investigation concludes "swap mechanical for operator-curated support," that change touches placement decision logic and requires Board approval per CLAUDE.md § 4 ("Things to ask before doing"). File the scoping memo first; let the Board decide.

**Cross-reference:** prior `P3 — Pink Box S/R confluence integration (NEW — 2026-05-10)` at BACKLOG.md L5257. Related but distinct: that entry is about ADDING operator-curated S/R as a tier-confluence boost; this entry is about VALIDATING/POTENTIALLY-REPLACING the HTF gate's existing support-detection mechanism. Both could be sequenced under a common operator-curated-S/R ingestion path if Item 3 (this entry) and Item 1 (P3 L5257) converge on the same ingestion sketch.

**Pickup conditions:** (a) ≥1 of the audit's two watch-items (W1 cvd_bull_flip @ N≥30, W2 BEAR/PREMIUM @ ≥3 distinct days) accumulates enough sample to re-cut; OR (b) operator decides the support-detection question is load-bearing independent of audit-window evidence. Don't pull this in just because it's interesting — pull it when the data justifies it.
...
13. **C-1 TASTYTRADE_PROVIDER_SECRET + TASTYTRADE_REFRESH_TOKEN rotation** — follow `runbooks/tastytrade_oauth_rotation.md` (the matched-pair atomic 2-step OAuth flow). Highest-complexity by procedural depth; runbook is canonical. **⚠️ ELEVATED P2 → P1 — 2026-05-29:** the refresh-token JWT was surfaced (truncated by pytest safe-repr) in the C-1 bitunix test-failure transcript earlier the same day (P2-verify annotation also on `c1-bitunix-cred-rotation` branch's `a868f46` commit). **Verification finding (2026-05-29):** the tastytrade Python SDK does **NOT** self-rotate the refresh token — `tastytrade.Session.refresh()` reads only `access_token` + `expires_in` from the OAuth response and updates only `self.session_token` + `self.session_expiration`; `self.refresh_token` is never written (RFC 6749 §6's optional new-refresh-token field is ignored). The `secrets.py`/`brokers/tastytrade.py` comments claiming per-session rotation were wrong — corrected on this branch. So the leaked JWT is **live until manually rotated**. **Exploit risk is partial** (transcript captured only head+tail of the JWT, body truncated by pytest safe-repr; also useless without the matching Client Secret — atomic pair). **Defer ceiling: 2 weeks (by 2026-06-12)** — schedule a focused session for the multi-step OAuth flow. **Load-path correction:** prod loads tastytrade creds from `/etc/trading-corp/tastytrade.env` (systemd EnvironmentFile drop-in), NOT KV — KV has NO `TASTYTRADE-*` secrets at all (runbook lines 52-57 are stale on the KV claim; runbook is no-edit-without-Board-approval, so flagging here). Rotation must update the env file, not KV. See `[[tastytrade-refresh-token-no-self-rotation]]`.
