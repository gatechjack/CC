...
## P2 — Polymarket WATCHLIST screen (`seed_polymarket_watchlist_deep`) selects on HELD-basis PnL, not realized — misranks; the option-(c) entry above overstates Phase 2 (filed 2026-07-26 via read-only PCT holding assessment)

**Verified (read-only, both `prod-live` AND `origin/main`, line 538):** `seed_polymarket_watchlist_deep.py` — the Sunday `watch_only_whales` job that surfaces the PROMOTE candidate pool — scores each whale's window on `compute_polymarket_stats` (HELD-to-resolution `Σ(1-price)·size if win else -price·size`), i.e. impostor #4. It does **not** use `build_audit_report` (REDEEM-grounded realized). **This contradicts the "Phase 2 … Same REDEEM-grounded compute" claim in the option-(c) entry above (≈L860):** only Phase 1 (`refresh_polymarket_whales`, the *selection* script) actually routes through `build_audit_report`; the Phase 2 *watchlist/candidate-discovery* screen was scoped to unify (L904, `whale_screening.py` exists) but its PnL basis was never moved — it still calls `compute_polymarket_stats` directly.

**Why it matters (proven, not theoretical):** re-scoring the current watchlist PROMOTE candidates on realized (`build_audit_report`, full exhausted window) **inverted the held-basis ranking**:
- **papuas** — held-window screen ranked it a top-3 promote (+$71k, WR 0.63, sharp); **realized PnL = −$61,264** (clean-hold decisions −$671k, clawed back by active trading). A sharp-looking, high-volume, held-"steady" money-loser.
- **viktorurolog16** — held-durability #1; **clean-hold realized −$10,287** (its +$28k realized is all exit-timing → not copyable by hold).
- **DegenKingBetter** — held-durability worst / "cooling"; **realized #1** (+$97.6k, 59% from clean holds, 0% favorite-farming).
- Only **{DegenKingBetter, ox1star84, CVCM}** have positive realized + value-creating exits + positive clean-hold edge (an edge copy-trading can actually capture). marchonnow already CUT by operator (realized −$3k, exits −$13.3k, 54% >85¢, inflation 1.06) — reproduced to the cent as the harness validation control.

**Fix shape (later, operator-queued — PCT is ON HOLD so not urgent):** route the seed's window scoring through `build_audit_report` (realized + clean-hold/partial-exit split + inflation ratio), matching the selection script — the watchlist is the promotion candidate source, so a held-basis screen there directly misleads any future promotion. Also correct the option-(c) entry's Phase-2 wording.

**Session artifacts (branch `claude`, off `prod-live` `e4219b3`, 5 commits, pushed):** `reports/2026-07-25_polymarket_copy_holding_assessment.md` (edge ~zero, one-whale concentration, roster algo-stale), `…_roster_actions.md` (KEEP/REMOVE/PROMOTE decision-support), `…_watchlist_durability.md` (2-snapshot durability — the held-basis test that MISSED this), `…_realized_decomposition.md` (this finding). **Decision-support only — no roster writes; the operator executes roster changes separately.** Related open items: new-whale full re-crawl did NOT complete (Cloudflare-blocked the 27k-cid batch-resolution endpoint; retry `seed_polymarket_watchlist_deep --dry-run` off-peak / smaller `--candidates`); roster action lists pending operator decision; the corrected algorithm has never been applied (live roster is hand-set 2026-07-07; `selection_metadata` algo output is 2026-05-11 pre-fix). Memory [[polymarket-copy-assessment-2026-07-25]]. **Priority: P2 screening-correctness; operator-queued; PCT on hold.**
