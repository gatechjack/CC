...
13. **C-1 TASTYTRADE_PROVIDER_SECRET + TASTYTRADE_REFRESH_TOKEN rotation** — follow `runbooks/tastytrade_oauth_rotation.md` (the matched-pair atomic 2-step OAuth flow). Highest-complexity by procedural depth; runbook is canonical. **⚠️ 2026-05-29:** a refresh-token-shaped JWT (truncated) was surfaced in the same C-1-bitunix transcript leak. **P2 — verify self-rotation behavior first:** `secrets.py` comments claim the refresh token "rotates per session," but `tastytrade_oauth_rotation.md` describes a *manual* rotation flow and frames the token as stable-per-Client-Secret (leans toward NOT self-rotating on use). Confirm via the Tastytrade SDK docs / a controlled session-exchange test. **If it self-rotates → the leaked JWT is already stale, file as no-action. If it does NOT → the leaked token is live → ELEVATE to P1 and rotate per the runbook.**
