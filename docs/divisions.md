...
| `kalshi_arbitrage` (structural) | Kalshi (shared `KalshiBroker` across kalshi_* divisions; broker:paper for equity tracking) | Two structural arb strategies (no LLM in path): **kalshi_tail_price_arb** (YES+NO at price tails ≤5¢ or ≥95¢) + **kalshi_temporal_bucket_arb** (P(early)>P(late) violations + bucket-sum<$1 detection). 5-min poll each. Multi-leg ProposedOrders share `kalshi_pair_id` or `kalshi_arb_set_id`. | Live paper, `enabled: false` until structural opps appear. Phase K2.1+K2.2 shipped. See `~/.claude/.../memory/trading_corp_kalshi.md`. |
