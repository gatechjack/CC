...
echo "===== A. Headline (excl. Climate/Weather + Crypto) ====="
...
SELECT COUNT(*) AS n,
 ...
 WHERE strategy='kalshi_llm_arbitrage'
   AND category NOT IN ('Climate and Weather','Crypto');
SQL

echo ""
echo "===== B. By category breakdown (the eligible set) ====="
sqlite3 -header -column $DB <<'SQL'
SELECT category,
       COUNT(*) AS n,
       SUM(won) AS w,
...
SELECT bucket, COUNT(*) AS n, SUM(won) AS w,
       ROUND(100.0*SUM(won)/COUNT(*),1) AS wr,
       ROUND(SUM(realized_pnl),2) AS pnl,
       ROUND(100.0*SUM(realized_pnl)/SUM(notional),1) AS roi_pct
  FROM rt
...
echo ""
echo "===== F. LLM calibration (excl. weather + crypto) ====="
sqlite3 -header -column $DB <<'SQL'
WITH rt AS (
  SELECT CASE
           ...
           ELSE 'p>0.80'
         END AS llm_band,
         market_result, won, realized_pnl, llm_prob
    FROM kalshi_round_trips
   WHERE strategy='kalshi_llm_arbitrage'
     AND category NOT IN ('Climate and Weather','Crypto')
     AND market_result IN ('yes','no')
...
echo ""
echo "===== G. Top losers + winners by event_ticker ====="
sqlite3 -header -column $DB <<'SQL'
...
echo ""
echo "===== H. Open positions (excl weather+crypto, from audit payload) ====="
sqlite3 -header -column $DB <<'SQL'
SELECT 'open_excl_weather_crypto' AS metric, COUNT(*) AS n
  FROM audit_event
 WHERE actor='kalshi_llm_arbitrage'
   AND kind='would_have_placed'
   AND COALESCE(json_extract(payload_json,'$.side'),'buy')='buy'
   AND COALESCE(json_extract(payload_json,'$.category'),'') NOT IN ('Climate and Weather','Crypto')
